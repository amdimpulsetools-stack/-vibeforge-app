...
interface PatientsClientProps {
  /** Página 0 sin filtros, traída por el Server Component (SSR) para que el
   *  primer render llegue con datos y sin spinner ni query del cliente. */
  initialFirstPage: { rows: PatientWithTags[]; count: number } | null;
}

export function PatientsClient({ initialFirstPage }: PatientsClientProps) {
  const { t } = useLanguage();
  ...
  // Catálogos (services + orígenes) cacheados 5 min por org: volver a la
  // página o reabrir los filtros no repite estos fetches. `enabled` espera a
  // la org — además corrige que antes el .or() de lookup_values se armaba con
  // la org aún sin resolver y perdía los orígenes propios de la organización.
  ...
  const { data: originsData } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await createClient()
        ...
        .order("display_order");
      return (data ?? []).map((v: { label: string }) => ({ label: v.label, value: v.label }));
    },
  });
  const originOptions = originsData ?? [];
  ...
  // Listado sobre React Query: la key incluye página + filtros, así que
  // volver a una combinación ya vista dentro del staleTime (5 min) renderiza
  // desde caché sin query ni spinner. Sin placeholderData: cambiar de página
  // o filtro no cacheado muestra el mismo spinner de siempre.
  const { data: listData, isPending: listPending, isFetching: listFetching } = useQuery({
    queryKey: [
      "patients",
      "list",
    ...
    queryFn: async () => {
      ...
      return {
        rows: (data as unknown as PatientWithTags[]) ?? [],
        count: count ?? 0,
      };
    },
  });
  ...
  return (
    /* Full-bleed en los dos ejes de breakpoint (pedido del founder: fuera
       ...
       altura se recalibra: topbar 4rem + solo el p-4 inferior = 5rem. El
       PatientDrawer, que en móvil sustituye a la lista, gana esos 32px.
       ...
       escritorio dvh == vh. */
    <div className="-mx-4 -mt-4 flex h-[calc(100dvh-5rem)] md:-mx-7 md:-mb-7 md:-mt-7 md:h-[calc(100dvh-4rem)]">
      {/* Main List */}
