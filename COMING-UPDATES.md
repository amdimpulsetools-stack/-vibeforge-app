...
| ✅ 4 | HECHO 13-ago (F1.5) | **Auditoría por usuario + reporte:** autor con fallback a email en cada movimiento, badge "digitado el X" anti-fecha-retroactiva, filtros por usuario/tipo/producto/fechas, KPIs del período, gráfica ventas-vs-compras, CSV e impresión. También del punch list: creador de categorías, precio compra + stock inicial en Nuevo producto, paciente vinculada por FK en salidas, densidad. Pendiente de esta línea: card "Por usuario" (mermas/salidas por persona, solo admin) → F2 | hecho |
| ✅ 5a | HECHO 13-ago | **Tabs Vencimientos y Rentabilidad reales** (adelantados de F2/F3): capital en riesgo por franja de vencimiento por lote; ganancia real por producto (ventas − COGS congelado), margen, capital en stock y plata dormida, con rango de fechas. Candado por plan Clínica pendiente para el lanzamiento público | hecho |
| 5 | Sep | F2: descuento desde cita (idempotente) + venta mostrador espejando patient_payments + selector de lote en salidas + cruce aplicación↔cobro (hoy las aplicaciones no suman ganancia). **+ Diferidos de la auditoría 13-ago**: COGS estampado en SERVIDOR (RPC/trigger — el snapshot de cliente puede congelar CPP viejo con dos usuarias simultáneas), tope de asignación automática de lote (evita saldos de lote negativos ocultos que sobreestiman el capital en riesgo), costo exigible en ajustes positivos grandes, descontinuar solo con stock 0, y agregado de kardex en servidor al acercarse al tope de 5,000 | 5-7 días |
| 6 | Sep/oct | F3: recetas por servicio, cierre mensual + caja conciliable, importador self-service del Excel (7 hojas) | 5-8 días |
