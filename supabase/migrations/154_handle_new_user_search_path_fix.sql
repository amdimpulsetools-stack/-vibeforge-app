-- ============================================================
-- 154: Fix handle_new_user trigger — signup "database error
--      saving new user" caused by unqualified function call.
--
-- The trigger is SECURITY DEFINER and was relying on whatever
-- search_path the caller had at signup time. From auth.signup
-- the search_path doesn't include public, so the unqualified
-- `seed_email_templates(new_org_id)` call failed with
-- "function seed_email_templates(uuid) does not exist".
--
-- Confirmed in prod auth logs (2026-05-13 03:59:38Z):
--   error_code: unexpected_failure
--   path: /signup
--   msg: 500: Database error saving new user
--   ERROR: function seed_email_templates(uuid) does not exist
--   (SQLSTATE 42883)
--
-- Fix: explicit SET search_path on the function + qualify the
-- function call with public. — defense in depth.
-- ============================================================
