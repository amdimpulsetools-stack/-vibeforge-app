-- ═══════════════════════════════════════════════════════════════════
-- MIGRATION 231: /reports usa el precio acordado de la cita (barrido 28-ago)
--
-- Hallazgo del barrido de consistencia fiscal: los reportes financieros
-- (get_reports_overview, mig 198) valorizaban las citas con el precio de
-- CATÁLOGO (services.base_price) mientras el dashboard (mig 200) usa el
-- precio ACORDADO (COALESCE(a.price_snapshot, s.base_price)). Con
-- descuentos o precios pactados, "Total Facturado" y "Pendiente cobro"
-- daban cifras distintas en /reports vs /dashboard para el mismo período.
--
-- Cambio: UNA línea del CTE range_appointments — el resto de la función
-- es VERBATIM de la mig 198. No es un tema de IGV (ambos montos son
-- brutos): es consistencia de base de precio entre pantallas.
--
-- Rollback: 231_reports_price_snapshot_rollback.sql (restaura la 198).
-- ═══════════════════════════════════════════════════════════════════
