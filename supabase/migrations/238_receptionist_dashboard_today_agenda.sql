...
--
-- 238: get_receptionist_dashboard v2 — agrega 'today_appointments'.
--
-- Feedback del piloto Vitra: el dashboard de la obstetra muestra su
-- "Agenda de hoy" en LISTA con estatus al costado; el de recepción solo
-- tenía los contadores (W1) y la lista de mañana (W2). Este bloque nuevo
-- lista las citas de HOY (sin canceladas, orden por hora) con:
--   - status               → badge (programada/confirmada/completada/no-show)
--   - is_recurring         → chip "Recurrente" (columna de patients, mig 096)
--   - price/paid_clinical  → chip "Falta pago S/ X" en el cliente
--
-- REGLA DE ORO (un número, una fórmula): precio y pagado por cita se
-- importan VERBATIM de la mig 233 (get_admin_dashboard_stats_v3):
--   precio = COALESCE(a.price_snapshot, COALESCE(s.base_price, 0))
--   pagado = SUM(patient_payments de la cita con
--            COALESCE(source,'clinical')='clinical')  ← jamás POS
-- Cobros en BRUTO (con IGV). El cliente solo resta para el chip.
--
-- El resto de la función es copia VERBATIM de la mig 236 (gating de rol
-- M12 incluido). Rollback: restaura la versión 236 exacta.
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- ── W1b (mig 238): Agenda de hoy en lista ──
    'today_appointments', (
      SELECT COALESCE(json_agg(json_build_object(
        ...
        'patient_phone', ta.patient_phone,
        'doctor_name', ta.doctor_name,
        'service_name', ta.service_name,
        'is_recurring', ta.is_recurring,
        'price', ta.price,
        'paid_clinical', ta.paid_clinical
      ) ORDER BY ta.start_time ASC, ta.id ASC), '[]'::json)
      FROM (
        SELECT
          a.id,
          a.start_time,
          ...
          COALESCE(p.is_recurring, false) AS is_recurring,
          -- Fórmula de la mig 233, VERBATIM — no reescribir jamás.
          COALESCE(a.price_snapshot, COALESCE(s.base_price, 0)) AS price,
          COALESCE((SELECT SUM(pp.amount) FROM patient_payments pp WHERE pp.appointment_id = a.id AND COALESCE(pp.source, 'clinical') = 'clinical'), 0) AS paid_clinical
        FROM appointments a
    ...
    -- canceladas) — VERBATIM mig 236/200. ──
    'daily_series', (
      SELECT COALESCE(json_agg(json_build_object(
        'date', ds.date,
        'count', ds.count
