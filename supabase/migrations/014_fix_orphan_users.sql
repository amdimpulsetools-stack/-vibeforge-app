-- =============================================
-- MIGRATION 014: Fix Orphaned Users
-- Users who registered but have no organization_members
-- record cannot pass RLS policies. This migration:
-- 1. Creates a self-healing RPC function
-- 2. Fixes existing orphaned users
-- 3. Re-applies the multi-tenant signup trigger
-- =============================================

-- =============================================
-- SECTION 1: Self-healing function
-- Called from frontend if user has no org membership
-- =============================================
...
-- =============================================
-- SECTION 2: Fix existing orphaned users
-- Create org membership for users who have none
-- =============================================
...
BEGIN
  FOR orphan IN
    ...
    WHERE om.id IS NULL
  LOOP
    org_name := COALESCE(orphan.raw_user_meta_data->>'org_name', 'Mi Clínica');
    new_org_id := gen_random_uuid();
    ...
    INSERT INTO organization_members (user_id, organization_id, role)
    VALUES (orphan.id, new_org_id, 'owner');
    ...
    SELECT id INTO cat_origin_id FROM lookup_categories WHERE slug = 'origin' AND organization_id = new_org_id;
    SELECT id INTO cat_payment_id FROM lookup_categories WHERE slug = 'payment_method' AND organization_id = new_org_id;
    SELECT id INTO cat_status_id FROM lookup_categories WHERE slug = 'appointment_status' AND organization_id = new_org_id;
    SELECT id INTO cat_responsible_id FROM lookup_categories WHERE slug = 'responsible' AND organization_id = new_org_id;
...
-- =============================================
-- SECTION 3: Re-apply the multi-tenant signup trigger
-- Ensures handle_new_user() is the updated version
-- =============================================
