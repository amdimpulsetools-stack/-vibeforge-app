-- ============================================================
-- Migración 217 — Farmacia (POS): confirmar y anular una venta
--
-- Pendiente de aplicar en producción (la aplica el orquestador).
--
-- La 216 dejó pharmacy_sales sin ninguna policy que permita salir de
-- 'borrador'. Confirmar una venta no es escribir una fila: es descontar
-- stock, congelar el costo, reservar un correlativo y cobrar, todo o
-- nada. Y anularla es devolver stock Y devolver dinero, también todo o
-- nada. Ese "todo o nada" es lo que vive aquí.
--
-- ESTAS DOS FUNCIONES SON EL ÚNICO CAMINO por el que el POS descuenta
-- stock y cobra. El navegador jamás inserta un inventory_movement ni un
-- patient_payment de farmacia: solo edita su borrador y llama a la RPC
-- con el UUID de ese borrador.
--
-- IDEMPOTENCIA: el UUID del borrador es la clave. Doble clic, reintento
-- de red o dos pestañas llaman con el mismo UUID; la primera confirma y
-- las demás encuentran la venta ya cerrada y devuelven EL MISMO
-- resultado, sin cobrar ni descontar de nuevo. El backstop de base es
-- el UNIQUE parcial de inventory_movements.sale_line_id (mig 213).
--
-- Ambas son SECURITY DEFINER con search_path fijado y GRANT explícito a
-- authenticated: los defaults se cerraron en la 193(d).
--
-- F4 NO EMITE COMPROBANTE ELECTRÓNICO. Nada aquí toca einvoices: la
-- venta se cierra con billing_mode='interno' y billing_status='no_aplica',
-- y lo que se entrega es una NOTA DE VENTA interna.
-- ============================================================

-- ── Costo promedio ponderado vigente, en servidor ───────────────
--
-- Réplica del criterio de avgCostByProduct (almacen/types.ts): media
-- ponderada de lo que ENTRÓ, nunca de lo que salió.
--
-- El detalle que no es obvio y que sí importa: los CONTRA-ASIENTOS
-- entran en la mezcla. Deshacer una entrada equivocada graba un 'ajuste'
-- negativo que hereda el costo de la entrada anulada; si se ignorara,
-- una entrada de 10@200 deshecha dejaría el promedio contaminado para
-- siempre (10@100 + 10@200 deshecha ⇒ CPP 150 en vez de 100). Con el
-- par completo, numerador y denominador se cancelan solos.
--
-- Sin entradas costeadas no hay promedio: cae al costo de la última
-- entrada conocida, y si tampoco hay, devuelve NULL (la línea se vende
-- igual, sin costo congelado — vender jamás se bloquea por no saber el
-- margen).
CREATE OR REPLACE FUNCTION pharmacy_avg_cost(p_product uuid)
RETURNS numeric
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  WITH costed AS (
    SELECT m.quantity AS q,
           COALESCE(m.unit_cost, o.unit_cost) AS c
      FROM inventory_movements m
      LEFT JOIN inventory_movements o
             ON o.id = m.reverses_movement_id
            AND o.movement_type = 'entrada'
     WHERE m.product_id = p_product
       AND (
         (m.movement_type = 'entrada' AND m.unit_cost IS NOT NULL)
         OR (m.movement_type = 'ajuste'
             AND COALESCE(m.unit_cost, o.unit_cost) IS NOT NULL)
       )
  )
  SELECT COALESCE(
    (SELECT round(sum(q * c) / sum(q), 4) FROM costed WHERE (SELECT sum(q) FROM costed) > 0),
    (SELECT m.unit_cost
       FROM inventory_movements m
      WHERE m.product_id = p_product
...
REVOKE ALL ON FUNCTION pharmacy_avg_cost(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION pharmacy_avg_cost(uuid) TO authenticated;
...
-- ── 2. Anular la venta ──────────────────────────────────────────
--
-- STOCK Y DINERO SE REVIERTEN JUNTOS O NO SE REVIERTE NADA. Si no hay
-- caja abierta donde registrar la devolución, la anulación entera se
-- rechaza: devolver las unidades al estante sin devolver el dinero deja
-- un descuadre que nadie va a encontrar después.
...
COMMENT ON FUNCTION pharmacy_avg_cost(uuid) IS
  'Farmacia F4: CPP vigente en servidor (réplica de avgCostByProduct). Incluye contra-asientos para que deshacer una entrada no contamine el promedio.';
