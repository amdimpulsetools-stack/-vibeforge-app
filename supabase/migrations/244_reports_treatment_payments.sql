...
--
-- 244: /reports — los cobros de tratamiento (mig 242) se reportan aparte.
--
-- Decisión del founder (2026-09-04): "Pendiente" de /reports es
-- Facturado(citas) − Cobrado; con cobros de tratamiento (que no tienen
-- cita) se volvía negativo. `payments_amount` excluye treatment_id y se
-- añade `treatment_payments_amount` para una card propia "Cobros por
-- tratamientos". Resto de la función VERBATIM de la 231.
-- Rollback: rollbacks/244_reports_treatment_payments_rollback.sql
  ...
  SELECT json_build_object(
    'totals', (
      SELECT json_build_object(
        ...
        -- Mig 244: los cobros de TRATAMIENTO (treatment_id, mig 242) salen de
        -- "Cobrado" de citas — si no, "Pendiente = Facturado(citas) −
        -- Cobrado" se vuelve negativo. Van en su propio total.
        'payments_amount', (
