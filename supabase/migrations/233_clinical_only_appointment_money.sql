-- ============================================================
-- Migración 233 — Dinero por cita: SOLO pagos clínicos
--
-- Pendiente de aplicar en producción (la aplica el orquestador).
--
-- EL PROBLEMA. La regla "la plata de farmacia no cancela deuda clínica"
-- (mig 213/219, filtro source='clinical') se respetaba en el saldo del
-- paciente pero NO en los cálculos POR CITA: una venta del POS vinculada
-- a una cita (patient_payments con source='pos' y ese appointment_id)
-- contaba como pago de la consulta en (a) la RPC de devolución al
-- cancelar — podía "devolver" o anular plata de farmacia — y (b) los
-- bloques de citas por cobrar del dashboard.
--
-- LA SOLUCIÓN. Copia VERBATIM de la mig 230 con un único cambio (buscar
-- "mig 233" / el filtro en los subselects): toda suma de pagos por
-- appointment_id añade COALESCE(source,'clinical')='clinical'.
-- El sidebar de la agenda aplica el mismo filtro en cliente desde esta
-- misma tanda. Los bloques de INGRESOS del dashboard no cambian: el
-- ingreso total sí incluye farmacia (es plata que entró).
-- ============================================================
...
BEGIN
  ...
  -- mig 233: solo pagos CLÍNICOS. Una venta de farmacia vinculada a la
  -- cita (source='pos') no es plata de la consulta: no puede devolverse
  -- ni anularse desde aquí (su camino es la anulación de la venta NV).
  ...
  IF v_caja_on THEN
    ...
    SELECT pp.id, pp.tender_kind INTO v_last_pay, v_tender
     ...
     WHERE pp.appointment_id = p_appointment_id
       AND COALESCE(pp.source, 'clinical') = 'clinical'  -- mig 233
  ...
  ELSE
    ...
    FOR v_pay IN
      SELECT id, amount
        FROM patient_payments
       WHERE appointment_id = p_appointment_id
         AND COALESCE(source, 'clinical') = 'clinical'  -- mig 233
  ...
  SELECT json_build_object(
    ...
    'pending_debt_month', (
      SELECT COALESCE(SUM(GREATEST(0,
        ...
        - COALESCE((SELECT SUM(pp.amount) FROM patient_payments pp WHERE pp.appointment_id = a.id AND COALESCE(pp.source, 'clinical') = 'clinical'), 0)
      )), 0)
      FROM appointments a
