...
> - **Recetas a recepción, Presupuestos sin paso intermedio y "Resumen de cobros" imprimible (v0.15.42, migs 256-260, PRs #353-#361 — 2026-09-09 a 09-11)**: **modalidad presencial/virtual por cita** (mig 256, selector inline en servicios "Ambos", plantilla de WhatsApp por modalidad); recetas con firma al pie, membrete sin razón social, "Dosis única", catálogos unificados y aviso opcional a recepción `prescription_issued` (mig 257 vínculo al catálogo; mig 258 endurece UPDATE de `prescriptions` a owner/admin/doctor); **"Iniciar tratamiento" directo desde un presupuesto pendiente** (mig 259); conceptos de pago con toggle y fix de la franja gris global (`sr-only` sin ancestro posicionado); **Reportes › "Resumen de cobros"** (mig 260: RPC `get_custom_report`, solo owner/admin): Servicios · Adelantos y pagos a cuenta · Farmacia · Cobros por tratamientos con Descripción/Cantidad/Precio/Total, TOTAL FINAL que cuadra con el Financiero (mismas CTEs de la mig 251), PDF con membrete. Diseño: `docs/spec-reporte-personalizado.md`. Detalle: Changelog v0.15.42.
...
- **Almacén (mig 267):** crear productos y editar el precio de venta es de owner/admin **y de cualquier miembro con el permiso `can_manage_inventory`**, que concede y quita **solo el owner** desde Admin → Equipo (interruptor "Puede gestionar el almacén"; el trigger de la 235 impide que un admin se lo autoconceda). Archivar, eliminar y restaurar (mig 264) siguen siendo owner/admin; entradas y salidas, cualquier miembro
...
| `treatments.outcome = 'completed'` (mig 268) | Cierre "Completado" sin desenlace clínico (ciclo de donante). CHECK recreado con nombre `treatments_outcome_chk` |
| `inventory_movements.treatment_id` (mig 268) | Aplicación hecha desde la ficha de un tratamiento. Nullable, FK **RESTRICT** (kardex append-only), índice parcial. Σ `cost_total` sin pares deshechos = costo de insumos del tratamiento (`treatmentSuppliesCost`, `lib/treatments/money.ts`). **Nunca es cobro**: fuera de `treatmentMoney`, Caja y deuda |
| RPC `treatment_apply_product(p_treatment_id, p_product_id, p_quantity, p_lot_id?, p_movement_date?, p_notes?)` (mig 268) | Salida `uso_en_cita` con `patient_id` de la paciente, `treatment_id`, **COGS = `pharmacy_avg_cost` (CPP vigente, servidor)**, sin precio. Cualquier miembro activo; exige addon fertilidad + módulo Almacén + tratamiento en curso. Stock insuficiente avisa, no bloquea (criterio POS). Devuelve `{movement_id, unit_cost, cost_total, warnings}` |
| RPC `treatment_undo_supply(p_movement_id)` (mig 268) | Contra-asiento (ajuste opuesto, `reverses_movement_id`, costo heredado). Editor de almacén (mig 267) o autor de la aplicación. Consumidores: `POST/DELETE /api/treatments/[id]/supplies` |
| `inventory_lot_changes` + triggers `inventory_lots_audit`/`inventory_lots_guard` + RPC `inventory_update_lot(p_lot_id, p_lot_code, p_expiry_date, p_reason)` (mig 269) | Corrección de código y vencimiento de un lote desde la ventana de lotes de Almacén. Editor de almacén (mig 267), motivo obligatorio. Auditoría automática de todo UPDATE (sea por RPC o directo). Un lote no cambia de producto ni de organización. No toca stock ni costos; el costo del lote no es editable |
...
| `prescriptions.medication_catalog_id` (mig 257) | Vínculo nullable (ON DELETE SET NULL) con `medication_catalog` que el compositor ya calculaba y descartaba. La V1 del aviso a recepción no lo lee; se guarda desde ya para que el puente con Farmacia (V2) arranque con historia |
| Policy `prescriptions_update` (mig 258) | UPDATE de recetas solo owner/admin/doctor (antes org-wide y sin `WITH CHECK`: recepción podía editar una receta firmada por PostgREST). Mismo criterio que el servidor aplica al crear |
| RPC `treatment_start_from_budget` (mig 259) | Admite `pending_acceptance` además de `accepted` y estampa `accepted_at`; "Iniciar tratamiento" directo desde Pendientes, sin "Marcar como aceptado" |
| RPC `get_custom_report(p_org_id, p_from, p_to, p_sections text[])` (mig 260) | "Resumen de cobros" de /reports. SECURITY DEFINER gateado a owner/admin (`get_user_org_role`, 42501). CTEs **verbatim** de `get_reports_overview` (mig 251): Servicios = `period_appointments`; Adelantos = `other_appointments + other + plans`; Farmacia = `pharmacy`; Tratamientos = `treatment_payments_amount`; Σ secciones == `payments_amount + treatment_payments_amount`. Precio real si es único, NULL + min/max si varía (nunca promedio). Devoluciones informativas fuera del total. Contrato `types/custom-report.ts`; consumidores `GET /api/reports/custom` y `GET /api/pdf/custom-report` |
| Helper `treatments_doctor_scoped(p_org_id)` + `treatment_close` / `get_treatments_overview` (mig 261) | Alcance por doctora del módulo Tratamientos en UNA condición: true solo para rol `doctor` sin `is_fertility_advisor`. Doctora + asesora ve e interactúa con todos los tratamientos (lista, detalle, KPIs, cierre); doctor a secas solo los suyos. Misma condición en `/api/treatments` y `/api/treatments/[id]` |
| Índice único parcial `whatsapp_config(phone_number_id) WHERE is_active` + RPC `captacion_summary(p_org_id, p_from date, p_to date)` (mig 262) | Un número de WhatsApp activo por org (el capturador resuelve la org por `phone_number_id`). Captación por **cohorte del primer mensaje** (fecha civil de la org) con citas/asistencias/cobros hacia adelante sin tope; lead = sin paciente previo (tolerancia 1 día); facturado solo clínico; fila orgánica; cruce por teléfono → paciente más reciente. Solo `service_role`; la API exige membresía activa y grant del addon |
| `captacion_ad_labels` (org × ad_id → label, RLS: lee miembro, escribe owner/admin) + `captacion_summary` devuelve `label` (mig 263) | Nombre propio que la clínica le pone a cada anuncio de Meta (el webhook solo trae ID y titular, no el nombre de la campaña). `PUT /api/captacion/ad-labels` |
...
3. **Resumen de cobros** (solo owner/admin, v0.15.42): reporte imprimible del rango con cuatro tablas Descripción · Cantidad · Precio · Total —Servicios, Adelantos y pagos a cuenta (incluye planes), Farmacia, Cobros por tratamientos— seleccionables por chips y un TOTAL FINAL que cuadra con "Cobrado total" del Financiero (mismas fórmulas, RPC `get_custom_report`, mig 260). Precio real o "varios (mín – máx)", nunca promedio; devoluciones informativas fuera del total; PDF con membrete de la org. Desde la mig 265, "Adelantos y pagos a cuenta" despliega el detalle por cobro (paciente, medio, motivo) y el PDF imprime la subtabla de abonos directos con la impresión auditada. Diseño: `docs/spec-reporte-personalizado.md`
...
| **App de Meta (developers.facebook.com)** | Expediente del Embedded Signup. App ID 1059167543290484 **publicada** (28-ago); verificación del negocio (AMD IMPULSE S.R.L.) **verificada** desde may-2024; **proveedor de tecnología verificado** (verificación de acceso aprobada 31-ago); Facebook Login for Business configurado (SDK JS, dominios, OAuth) y Configuración de Embedded Signup creada (config_id 4454930594731788, plantilla token 60 días — aviso/renovación pendiente como tanda). Falta SOLO el **App Review** de `whatsapp_business_management/messaging` (acceso avanzado): screencast obligatorio + Data Use Checkup — paquete completo en `docs/meta-app-review.md` (justificaciones EN, guion del video plan B con número de prueba, respuestas de datos). El popup bloquea el registro de clientes hasta la aprobación (verificado: no es cuenta ni navegador) | **App Review APROBADO 15-sep-2026** (`whatsapp_business_messaging`, `whatsapp_business_management`, `public_profile`) + **registro como Independent Tech Provider completado el mismo día** (era el paso que faltaba: sin él el popup decía "no puede registrar clientes"). Embedded Signup Builder: los tres requisitos en verde; el flujo llega a la selección de portafolio en **v4** sobre la config 4454930594731788. Código v4 + Coexistence sin `phone_number_id` + CSP `staticxx.facebook.com` (PR #365). Pendiente: cuenta sandbox de Meta y primera conexión de prueba desde Ajustes; luego primer número real de una clínica (lo conecta la doctora con su propio Facebook, admin de su portafolio); aviso de caducidad del token a 60 días |
