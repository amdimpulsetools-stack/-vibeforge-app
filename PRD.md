...
| **Sentry** | Monitoreo de errores en cliente, servidor y edge | **Integrado pero inactivo** (auditoría 22-sep): sin `SENTRY_DSN` / `NEXT_PUBLIC_SENTRY_DSN` el SDK no inicializa y no llega ni un evento. Antes de encenderlo: (a) el ingest de Sentry **no está en `connect-src`** de la CSP — el cliente quedaría mudo en silencio; se resuelve con `tunnelRoute` (ruta propia, hay que añadirla a `publicPaths`) o allowlist del dominio; (b) poner el DSN **enciende la subida de source maps**, que necesita `SENTRY_AUTH_TOKEN` + `SENTRY_ORG` + `SENTRY_PROJECT`. Reparto: credenciales por la integración de Vercel, configuración de código en el repo — **nunca `npx @sentry/wizard`** (sobrescribe los configs afinados en v0.15.27). Detalle: Changelog v0.15.43 |
...
SENTRY_AUTH_TOKEN=        # Source maps: lo inyecta la integración de Vercel (pendiente)
SENTRY_ORG=               # Source maps (pendiente)
SENTRY_PROJECT=           # Source maps (pendiente)
SMTP_HOST=
SMTP_PORT=
SMTP_USER=
SMTP_PASS=
