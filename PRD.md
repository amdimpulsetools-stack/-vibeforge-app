...
> - **App Review de Meta APROBADO (15-sep-2026)**: los tres permisos (`whatsapp_business_messaging`, `whatsapp_business_management`, `public_profile`) con acceso avanzado el mismo día del envío. El expediente de WhatsApp Cloud API queda completo; el Embedded Signup puede registrar clínicas reales. Siguiente: primer número real + validación de `register`/`postMessage` (PR #327) + aviso de caducidad del token (60 días). Detalle: `docs/meta-app-review.md` §5.
