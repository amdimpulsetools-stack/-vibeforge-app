...
> **Versión:** 0.15.19
> **Estado:** MVP en producción + Sistema de módulos verticales (addons) + Primer vertical OMS (curvas de crecimiento pediátrico) + Portal del Paciente Phase 1 + Dashboard admin con timeline + Pacientes: etiqueta Recurrente + /book redesign + Presupuestos de tratamiento + Descuentos + Consentimiento informado Tier 1 (Ley 29414) + Facturación electrónica SUNAT vía Nubefact (MVP) + Google Calendar (org-level, one-way) + Rediseño UX Modal Historia Clínica + Consultorios autorizados por doctor + Pricing alineado: S/129 / S/349 / S/649 + Frecuencia semestral (8.3% off) en checkout MP + Reporte IA Avanzado capas 1+2 documentado (v0.13.4) + Perfil de Organización + helper de membrete + plantillas PDF (receta/nota/exámenes) usando membrete + wizard Nubefact pre-llena RUC desde org + hardening legal Ley 29733 con rediseño visual de /terms y /privacy (v0.14.0) + Aceptación explícita de Terms+Privacy en registro (mig 116) + Ubigeo SUNAT en organizations (mig 117) + Bloqueo de sesión para miembros desactivados (mig 118) + Especialidad editable + tabla doctor_specialties (mig 119) + Consentimiento informado digital MVP (mig 120) + UX unificada de consentimientos manuales + digitales (mig 121) (v0.14.1) + Hardening de seguridad post-review: TERMS_VERSION server-side + RPC get_user_id_by_email (mig 122) + storage UPDATE policy + signature_data CHECK + middleware terms-acceptance gate (mig 123) + HC: DNI+edad en header, label "Motivo de consulta (Subjetivo)", layout SOAP vertical + Múltiples diagnósticos CIE-10 por nota con chips removibles (mig 124) + Código CIE-10 manual one-off + Toggle Nota | Timeline en modal HC con cards colapsables y filtro asc/desc + Medicamentos y exámenes dentro de cada card del Timeline (snapshot clínico completo por consulta) (v0.14.2) + Primer addon vertical end-to-end: Fertilidad (Pack Básico) — Seguimientos automatizados con atribución honesta de recuperaciones, journey state machine, integración con WhatsApp Business API existente + 5 plantillas peruano + cron diario (mig 127-131) + tier_group para mutua exclusión basic/premium en addons + UX redesign Settings/Módulos al estilo /integraciones (Activar → modal → engranaje config) + Empty states guiados en /admin/services para onboarding de owner nuevo + Slug de plan consolidado (mig 132 fix precio Independiente + mig 133 consolidación 'starter' → 'independiente') (v0.15.0) + Settings/Integraciones agrupado por status + Break time default OFF + Empty states /admin/doctors /admin/offices /patients /scheduler + Tour interactivo driver.js + Hardening completo del flujo de invitación (mig 134) + UNIQUE INDEX doctors (mig 135) + Account header reactivo + WhatsApp clipboard tab desbloqueado + Hero landing con gradient animado (v0.15.1) + **Card Centro Médico con borde gradient animado + glow pulsante + botón CTA tipo Brief IA con shimmer + Sección "Otras especialidades disponibles" oculta si la org tiene specialty principal (verticales más limpias) + Fix lectura de `patients.sex` en SELECT del drawer (campo sí persistía pero no se leía) + Fix KPI revenue atribuido solo cuenta categoría A (withContact) — honestidad de la métrica del módulo de seguimientos + Embudo de presupuestos del Pack Fertilidad: tabla `budget_records` (mig 136) independiente de `treatment_plans` (Vitra usa Omnia para HC, no necesitan plan en Yenda para registrar envío) + 7 tipos de tratamiento (FIV/IIU/Inducción/Crio/Ovodonación/ROPA/Otro) + monto opcional + flag `is_fertility_advisor` en organization_members (mig 137) para rol obstetra/asesora con permisos extendidos + vista nueva `/scheduler/budgets` con kanban (Pendientes/Aceptados/Rechazados) + 4 KPIs (enviados 30d, % aceptación, % rechazo, tiempo promedio aceptación en días) + tracking abierto indefinidamente (3+ meses entre envío y decisión es realista) (v0.15.2)** + Preparación de pilot con primer cliente real (Vitra, fertilidad) + **Topbar rediseñada con dropdown hover (avatar + nombre opcional, items Cuenta/Configuración/Cerrar sesión) y animación Framer Motion suave + Animaciones de modales/popovers/sheets/dropdowns/alert-dialogs unificadas con keyframes CSS reales (mismo cubic-bezier(0.16, 1, 0.3, 1) que el dropdown del topbar) reemplazando utilidades muertas de `tailwindcss-animate` y honrando `prefers-reduced-motion` + Plantillas WhatsApp clipboard multi-kind (mig 139): tabla `org_whatsapp_clipboard_templates` con 3 tipos (`post_appointment`, `second_consultation_followup`, `budget_followup`), seed function + backfill retroactivo (post_appointment para todas las orgs; kinds de seguimiento solo para orgs con `fertility_basic`/`fertility_premium`) + API `/api/whatsapp-clipboard-templates` (GET all kinds, PUT one, admin/owner only) + lib `lib/whatsapp-clipboard-config.ts` refactorizado a multi-kind con `buildMessage(kind, template, vars)` type-narrowed (legacy single-template exports preservados para retrocompatibilidad) + Settings UI reescrito con tabs por kind, addon-gated, toolbar B/I/S con marcadores WhatsApp (`*bold*`/`_italic_`/`~strike~`), picker de ~25 emojis curados, inserción cursor-aware y vista previa que parsea formato WA → bold/italic/strike HTML + Card de seguimiento con teléfono visible formato `+51 987 654 321` clickeable como wa.me + dual buttons (Enviar por WhatsApp wa.me / Copiar mensaje) device-aware (móvil → wa.me primario; desktop → Copiar primario porque las recepcionistas mantienen WhatsApp Web abierto en otra pestaña y un wa.me fresco la recarga) + kind elegido por card (`budget_followup` si hay budget linkeado, `second_consultation_followup` si no) + cache de promesas a nivel módulo deduplicando fetches de plantilla entre todas las cards + Modal post-cita también con dual buttons device-aware (Copiar + wa.me) y teléfono propagado desde el form de cita + **Phase 1 de optimización de performance en `/scheduler/follow-ups` y `/scheduler/budgets`**: eliminado double-fetch en mount de followups (de 6 counts en cold start a 3) + `/api/budgets` GET reorganizado de ~6 round-trips secuenciales a 3 olas con `Promise.all` (membership+addon-check, listing+counts+kpi, senders) + removido `count: "exact"` redundante del listing usando `limit+1` con `.range()` para `hasMore` + columnas whitelisted en ambos endpoints (drop de campos pesados sin uso como `contact_events JSONB`, `notes`) + dropdown de doctores filtra solo `id, full_name` desde el cliente (TTFB estimado: −50–70 % en `/scheduler/budgets`, −30–40 % en `/scheduler/follow-ups`) + Research deliverables en `docs/research/`: `openpay-pe-evaluation.md` (propuesta de abstracción multi-gateway, recomendación de iniciar con Culqi por Yape nativo y Openpay PE solo en segundo lugar para B2B con relación BBVA) y `perf-followups-budgets.md` (diagnóstico completo con Phase 2 — indexes + React Query — y Phase 3 — KPIs materializados + server components — recomendados) + Branch fixes en el camino: dialog flicker top-left por colisión `translate` de Tailwind v4 con `transform` del keyframe; route export error en `/api/whatsapp-clipboard-templates` por non-handler exports prohibidos en `route.ts` de Next.js 15 (v0.15.3) + Cascada de 3 intentos en seguimientos (Vitra Sprint 1): endpoint único `POST /api/clinical-followups/[id]/advance` con discriminated-union Zod (mark_contacted/pospuesto/agendado/cerrar_sin_respuesta), append a `contact_events`, overflow → auto-close + chip "Intento N/M" + 4 botones de cascada en la card (v0.15.4) + **Phase 1 (foundation) de Budget Tiers A/B/C del addon Fertilidad**: migración 140 (`services.is_budget_eligible` + tabla `service_budget_tiers` con UNIQUE parcial `WHERE is_active=true` para soft-delete + recreación + extensión `budget_records` con `service_id`/`tier`/`assigned_at`/`assigned_by_user_id` y drop de NOT NULL en `sent_at`/`sent_by_user_id` + backfill best-effort de `service_id` por ILIKE de `services.name` vs `treatment_type` legacy; `treatment_type` se conserva como snapshot denormalizado y los reports deben usar LEFT JOIN) + helper `useFertilityAddon()` y componente `<FertilityAddonGate>` reusables (eliminan el inline `hasAnyAddon([FERTILITY_BASIC_KEY, FERTILITY_PREMIUM_KEY])` en futuros call-sites, los 6 actuales se migrarán naturalmente en fases siguientes) + fix UI audit §3.2: `/scheduler/follow-ups` ahora tiene gate de UI (early return con CTA a `/settings?tab=modulos`) y nav entry con `requiresAnyAddon` (sidebar) + null-guards en `daysAgo`/`daysBetween` y en los renders de `sent_at` ("Sin enviar" en lugar de "Invalid date") por la nueva nullability de `sent_at` + nuevo hook `useOrgFertilityAdvisors()` + endpoint addon-gated `GET /api/admin/fertility/advisors` (lista miembros con `is_fertility_advisor=true`, listo para el dropdown asesora de Phase 3) (v0.15.5) + **Phase 2 (UI admin) de Budget Tiers A/B/C del addon Fertilidad**: nuevo endpoint `GET|PUT /api/services/[id]/tiers` (admin/owner only, addon-gated, upsert por id existente para preservar la UNIQUE parcial `WHERE is_active=true`) + UI en `/admin/services` (gateada con `<FertilityAddonGate>`): checkbox "Habilitar para presupuestos del addon Fertilidad" en el form, sub-sección expandible con 3 filas A/B/C (monto + moneda PEN/USD + qué incluye + notas internas + toggle activo) y botón explícito "Guardar tiers" + badge "Addon Fertilidad" (lucide Sparkles, emerald) en la lista de servicios para `created_by_addon IS NOT NULL` + botón "Eliminar" oculto para servicios addon-managed (defense in depth: trigger DB ya bloquea + UI no lo ofrece) + activación de addon (`POST /api/addons/[key]/activate`) ahora invoca `seed_fertility_services(p_org_id)` solo en la PRIMERA activación (count de servicios con `created_by_addon='fertility_basic'` antes de seedear; idempotente pero evita reruns innecesarios) (v0.15.6) + **Phase 3 (modal + entry points + sub-bucket) de Budget Tiers A/B/C del addon Fertilidad**: componente reusable `<AssignBudgetModal>` (autogateado con `<FertilityAddonGate>`) que carga servicios elegibles vía nuevo endpoint addon-gated `GET /api/services/budget-eligible` (servicios con `is_budget_eligible=true AND is_active=true` + tiers activos sorteados A→B→C) y asesoras vía `useOrgFertilityAdvisors()`, con tier picker A/B/C como cards con monto/moneda/incluye visibles + dropdown asesora (incluye "Sin asignar") + textarea notas; submit a nuevo `POST /api/budgets/assign` (addon-gated; recepcionistas 403; lookup de tier para snapshot de monto+currency; insert en `budget_records` con `assigned_at=NOW()`/`assigned_by_user_id=auth.uid()` y `sent_at=NULL`/`sent_by_user_id=NULL`; `treatment_type` inferido vía mapping documentado de `services.name` → enum FIV/IIU/INDUCCION/CRIO/OVODONACION/ROPA/OTRO con TED→OTRO porque el enum no tiene aún valor TED) + tres entry points wireados: appointment-sidebar (botón Asignar presupuesto solo en status `completed` y addon activo, oculto a recepcionistas), `FertilityBudgetRecordsSection` del patient-drawer (botón sustituye al legacy "Registrar presupuesto enviado" como ruta default), followup-card (`pending`/`no_response` y `linkedBudget === null`, con `onBudgetAssigned` cascada hasta `refresh()` en page.tsx) + sub-bucket "Sin procesar" en `/scheduler/budgets` Pendientes: endpoint `/api/budgets` ahora devuelve `counts.pending_unsent` y `counts.pending_sent`, UI splitea en dos sub-secciones con badges amber (Sin procesar) y blue (Enviado, esperando respuesta), card en Sin procesar gana botón "Enviar al paciente" que llama nuevo `POST /api/budgets/[id]/send` (rellena `sent_at=NOW()`/`sent_by_user_id=auth.uid()`; addon-gated; valida `pending_acceptance AND sent_at IS NULL`; TODO Phase 4 para PDF + dispatch) + decisión documentada: `asesora_id` en body de `assign` es validada contra `organization_members` pero NO PERSISTIDA todavía porque `budget_records` no tiene columna dedicada; sobrecargar `sent_by_user_id` rompería el split Sin procesar/Enviado, por lo que Phase 4 debe agregar `assigned_asesora_member_id` (v0.15.7) + **Phase 4 (final) de Budget Tiers A/B/C del addon Fertilidad — generador de PDF + storage + cleanup**: migración 141 con 3 columnas nuevas en `budget_records` (`pdf_storage_path`/`pdf_generated_at`/`pdf_size_bytes`) + bucket privado `budget-pdfs` con RLS path-based (org_id como primer folder, `get_user_org_ids()` para SELECT/INSERT/UPDATE, `is_org_admin()` para DELETE; mismo shape que mig 015a/120/123) + fix retroactivo flagged en `docs/research/derma-photos-storage-strategy.md`: el bucket `clinical-files` referenciado por `app/api/clinical-attachments/route.ts:80` nunca se creó por migración (solo manualmente en prod), agregado en mig 141 con la misma RLS para que envs nuevos no fallen + `<BudgetPdfDocument>` (`@react-pdf/renderer`, Helvetica default — fonts custom diferidos a polish) con header band emerald + barrita accent + título PRESUPUESTO + tabla 2 cols (paciente/documento/tratamiento/tier vs fecha/médico/asesora/vigencia) + card de servicio con "PAQUETE A|B|C" y "Qué incluye" derivado de `tier.includes_text` con fallback genérico por treatment_type + total box emerald + 4 boilerplate Consideraciones (vigencia 90d, 60d post-pago, retención 10%, servicios extra cotizados aparte) + footer con RUC/firma/asesora + helper `lib/budget-pdf/generate.ts` con cache: si `pdf_generated_at >= updated_at` reutiliza el archivo existente y solo emite signed URL fresca, si no renderiza→sube→persiste + `POST /api/budgets/[id]/pdf` (addon-gated, runtime nodejs) que devuelve `signed_url` con TTL 1h + wired en `[id]/send` para auto-generar al marcar como enviado y devolver `pdf_signed_url` en la respuesta (no fatal: si el render falla el row queda enviado y el cliente puede reintentar) + botones "Descargar PDF" en budget-card del kanban y en cada row de `<FertilityBudgetRecordsSection>` (lucide `FileDown`, gateado contra `acceptance_status='expired'` forward-compat) + cron mensual `/api/cron/budget-pdf-cleanup` (schedule `0 8 1 * *` UTC = 3 AM Lima día 1, gateado con `CRON_SECRET`) que purga PDFs de rejected >6m, accepted >2y, expired >6m: borra del bucket via service-role + nullea las 3 columnas en la fila + log de counts (v0.15.8) + **Phase 5 prep — treatment lifecycle + auto-followup**: migración 142 extiende `budget_records.acceptance_status` de 3 a 6 estados (`pending_acceptance` / `accepted` / `in_progress` / `completed` / `rejected` / `expired`) + 3 columnas nuevas (`started_at` / `started_by_user_id` / `completed_at`) + index parcial `idx_budget_records_accepted_unstarted` para el predicado del cron + nuevo `followup_rules` per-org `fertility.budget_accepted_pending_start` (delay 14d, max_attempts 3, target `fertility.treatment_started`) seedeado para todas las orgs con addon activo via `INSERT ... SELECT FROM organization_addons` con `ON CONFLICT (organization_id, rule_key) DO NOTHING` + 2 endpoints nuevos: `POST /api/budgets/[id]/start` (owner/admin/doctor/advisor; valida `accepted` + `started_at IS NULL`; setea `acceptance_status='in_progress'` + `started_at=NOW()` + `started_by_user_id=auth.uid()`; cierra el followup `budget_accepted_pending_start` del paciente con closure `agendado_via_contacto` si había `first_contact_at`, else `agendado_organico_dentro_ventana`, append `treatment_started` event), `POST /api/budgets/[id]/complete` (admin/owner only — financial-impact decision; valida `in_progress`; setea `acceptance_status='completed'` + `completed_at=NOW()`) + transformación de followups upstream en `POST /api/budgets/assign`: si el body trae `followup_id` y la fila linkeada tiene `rule_key IN ('fertility.first_consultation_lapse', 'fertility.second_consultation_lapse')` Y `closed_at IS NULL`, se transforma in-place a `fertility.budget_pending_acceptance` (`target_category_canonical='fertility.treatment_initiated'`, reset `attempt_count=0` + `snooze_until=NULL`, status `pospuesto→pendiente`, `expected_by=NOW()+90d`, append `rule_transition` event con from_rule/to_rule/budget_record_id) **preservando `first_contact_at`** (señal de atribución honesta Categoría A vs B) + linkea el budget al followup transformado (dedup verificado: `maybeCreateBudgetPendingFollowup` ya hace lookup por rule_key antes de insertar) + extensión del cron diario `fertility-followup-contact`: nuevo paso post-loop que itera orgs activas, busca `budget_records WHERE acceptance_status='accepted' AND started_at IS NULL AND accepted_at <= NOW()-delay_days`, idempotencia vía búsqueda de followup abierto con `rule_key='fertility.budget_accepted_pending_start'` y `contact_events[].budget_record_id` matching (evita duplicados), inserta clinical_followup nuevo con `priority='yellow'`, `expected_by=NOW()+7d`, `reason='Verificar inicio de tratamiento'`, `attempt_count=0`, `max_attempts=3` (sourced from rule), `contact_events=[{type:'budget_accepted_pending_start_alert',budget_record_id,at}]` + helper `resolvePatientDoctor` para best-effort doctor_id (recent appointment → fallback any active doctor) + 3 sub-buckets visuales en `/scheduler/budgets` columna Aceptados (mirroring del patrón `<PendingSubGroups>`): "Por iniciar" (`accepted_unstarted` con badge amber, urgency tint en card por edad de `accepted_at`: <7d gris / 7-14d amber-recordar / >14d rose-verificar), "En curso" (`in_progress` badge azul, botón **Marcar completado** admin/owner only via `useOrgRole().isAdmin`), "Completados" (badge emerald read-only, muestra `completed_at`) + nuevo badge tab Aceptados con count amber si `accepted_unstarted > 0` + `useOrgRole()` consumido en card para gating UI del Marcar completado + `/api/budgets` GET extendido: el bucket `accepted` ahora retorna `acceptance_status IN (accepted, in_progress, completed)` con `.in()` filter, response añade `counts.accepted_unstarted` (= legacy `counts.accepted`), `counts.in_progress`, `counts.completed`, `BUDGET_COLUMNS` whitelist incluye `started_at` / `started_by_user_id` / `completed_at` + extensión de `BudgetAcceptanceStatus` y `BudgetRecord` en `types/fertility.ts` + `ContactEventType` extendido con 3 nuevos tipos (`rule_transition` / `treatment_started` / `budget_accepted_pending_start_alert`) y `ContactEvent` con campos opcionales `from_rule` / `to_rule` para rule_transition events + `ruleStepperActiveIdx()` en followup-card mapea `fertility.budget_accepted_pending_start` al stepper dot 2 (mismo stage que `budget_pending_acceptance` — ambos son fase post-segunda-consulta) + `buildReasonForTarget()` agrega caso `fertility.treatment_started` → "Verificar inicio de tratamiento" (v0.15.9) + **Pre-launch MercadoPago hardening — Wave 1 (grace period + self-serve cancellation)**: migración 144 extiende `organization_subscriptions.status` con literal nuevo `grace` + 5 columnas (`grace_period_until` / `last_payment_failure_at` / `failure_count` / `past_due_since` / `cancelled_by_user_id`) + tabla `billing_events` append-only para auditar transiciones (event_type entre `payment_failed`/`entered_grace`/`grace_extended`/`grace_expired`/`recovered_to_active`/`past_due_terminated`/`cancelled_by_user`/`cancelled_by_mp`/`email_sent`) + RPC `get_user_session_check` redefinida para aceptar `status='grace' AND grace_period_until > now()` Y `status='cancelled' AND mp_next_payment_date > now()` (cancelado con período pagado vigente) — esto cierra el P0 del audit donde `past_due` expulsaba al usuario instantáneamente + webhook `/api/mercadopago/webhook` reescrito en `handlePaymentFailure`: primer rejected → `grace` + `grace_period_until=NOW()+7d` + `failure_count=1` + email "Pago no procesado", siguientes fallos durante gracia → `failure_count++` sin extender ventana, `failure_count>=4` (cubre 3 retries MP + final settlement) → flip directo a `past_due`; en `approved` se resetea `failure_count=0` + clear grace y log `recovered_to_active` + nuevo cron diario `/api/cron/billing-status` (schedule `0 14 * * *` UTC = 09:00 Lima, gateado con `CRON_SECRET`) que: (1) flip `trialing` con `trial_ends_at<=NOW()` → `past_due` + log; (2) en `grace` con expiry a ≤2d envía recordatorio "Tu período de gracia termina en 2 días" (idempotencia: skip si hay `email_sent` con kind=grace_ending en últimas 36h); (3) en `grace` con expiry vencida → flip a `past_due` + `past_due_since=NOW()` + email "Acceso suspendido"; (4) `past_due` con `past_due_since<=NOW()-30d` → flip a `cancelled` definitivo + `lib/billing-emails.ts` con 3 templates en español (payment_failed/grace_ending/access_suspended) reusando `buildEmailHtml` y `sendEmail` (Resend), audit log automático en `billing_events.metadata.kind` + helper `resolveBillingEmailContext(supabase, orgId, subId)` que consolida lookup de email del owner (`mp_payer_email` con fallback a `user_profiles.email` del owner) + nombre de org + plan + monto formateado `S/ X.XX` + fecha grace `DD/MM/AAAA` + nuevo endpoint `POST /api/billing/cancel` (owner only, paymentLimiter): llama `preApproval.update({status:'cancelled'})` en MP primero (abort si falla — local + MP deben quedar sincronizados); stampa `status='cancelled'` + `cancelled_at=NOW()` + `cancelled_by_user_id=auth.uid()` via admin client; log `billing_events` con `previous_status` + `access_until=mp_next_payment_date`; respuesta `{ok, access_until, message}` — el acceso NO se corta instantáneamente porque la RPC del middleware mantiene viva la fila `cancelled` mientras `mp_next_payment_date > now()` (usuario conserva acceso hasta el fin del período pagado) + reemplazo del botón "Eliminar cuenta" (sin handler) en `/account` Danger Zone por **"Cancelar suscripción"** owner-only con `<ConfirmDialog>` explicando los 3 garantías (acceso hasta fecha X, datos 90 días, reactivación posible) + estado read-only post-cancel "Cancelada — acceso hasta {date}"; admins/recepcionistas ven mensaje "Solo el propietario puede cancelar"; entrada "Eliminar cuenta" diferida a iteración futura (es flow distinto: borrar org vs detener cobro) + indexes parciales `idx_org_subs_grace_expiry WHERE status='grace'` y `idx_org_subs_past_due_since WHERE status='past_due'` para que el sweep del cron sea barato + 5ª entrada en `vercel.json` (`/api/cron/billing-status` schedule `0 14 * * *`) + **Wave 2 (pendiente, no iniciada)**: sincronizar `POST /api/plans` con `preApproval.update` en MP al cambiar plan (hoy actualiza DB pero MP sigue cobrando monto anterior) + endpoint de reactivación self-serve para cancelladas dentro de 90d + refund flow desde admin (v0.15.11) + **Members + invitations hardening**: `professional_title` migrado a `organization_members` (per-org, mig 146) — la misma persona puede ser Dr. en una clínica y Lic. en otra sin leak global + RPC `accept_invitation` reescrita para no expulsar al invitado de sus otras clínicas (solo elimina default orgs sole-member) + nueva RPC `update_my_professional_title` `SECURITY DEFINER` para que doctores editen su propio título desde `/account` sin abrir UPDATE policy y permitir escalación de rol + callback de auth ya no auto-acepta invitaciones: redirige a `/register?invite=TOKEN` con dos botones reales (Aceptar / Rechazar) + endpoint nuevo `POST /api/invitations/[token]/reject` (auth + email match obligatorios) + status `'rejected'` agregado al CHECK de `organization_invitations` (mig 147) + cleanup en prod de membresía/doctor duplicados de Vanessa creados durante el repro del bug (v0.15.12) + Signup fix + founder dashboard <1s + device session limits anti-sharing (mig 156) + 2FA opcional + plan soft-wall members/offices + clinical access log NTS 139 (v0.15.13–0.15.17, ver changelogs) + Presupuestos FIV PDF con plugins per-org (migs 167-169) + Estado de cita en vivo (migs 170-171) + Tab Fertilidad en /reports + canales reales de envío de presupuestos (mig 172) + email de presupuesto editable (mig 173) + **Lanzamiento de los 2 primeros pilotos comerciales (Vitra + Dra. Quispe, 2026-07-03; trials extendidos al 07-sep)** + Ajuste de honorarios por presupuesto (mig 174) + Landing auditada (copy/UX/neuromarketing) + saneamiento /producto y /blog (REPLACE→Yenda, testimonios placeholder fuera) + fix $ duplicado con S/ (Coins) + notificaciones de límite solo owner/admin + saludo consciente de honoríficos + **Owner-doctor: el owner/admin crea su propio perfil de doctor (`POST /api/doctors/self`, pricing-neutral)** + **Dashboard de Asesora para `is_fertility_advisor` (embudo: seguimientos + presupuestos + agenda del día)** + asesoras agendan para cualquier doctor (v0.15.18) + **Agenda con precisión de 15 minutos**: `scheduler_settings` gana `start_minute`/`end_minute` (mig 175, aplicada en prod; la ventana acepta p.ej. 07:15, invariante open<close a nivel de minutos, grilla de paso uniforme desde `start_hour:start_minute`) + Settings→Agenda con selects de 15 min y resumen de duración + modales validan a nivel de minuto + **el card de cita adopta el color del estado en vivo** (fondo azul cuando llegó, verde en consulta, gris al finalizar — no solo el pill) + Landing: eliminada la sección de roadmap público, nueva sección "Yenda no deja de crecer" post-pricing con teasers Próximamente (inventario, caja, HC Premium de Fertilidad, estadísticas por asesora, más medios de pago, IA) + copy actualizado (Excel como competidor real, precios S/ sin duplicar $) + roadmap interno: `docs/coming-updates-core.md` gana "Dos atenciones en el mismo bloque de horario" (decisión pendiente 2026-07-10) (v0.15.19)
...
## Changelog — Sesión 2026-06-12 tarde (v0.15.14) — Fixes post-launch del envío + email de presupuesto editable
...
## Changelog — Sesión 2026-07-03 (v0.15.15) — Lanzamiento de los dos primeros pilotos + cierre del email de presupuesto
...
## Changelog — Sesión 2026-07-07 (noche) — Fixes UI ($→S/) + notificaciones de límite + operativo pilotos (v0.15.16)
