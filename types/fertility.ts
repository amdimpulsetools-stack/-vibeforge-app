/**
 * Types for the fertility addon — presupuestos, tiers y catálogo de
...
/**
 * ── Tipos genéricos de seguimientos ────────────────────────────────
 * Se mudaron a `types/followups.ts` (el módulo de seguimientos es core
 * desde la mig 182 y ya no es del addon). Se re-exportan desde aquí
 * para no romper los imports existentes; el código nuevo debería
 * importar de `@/types/followups`.
 */
export type {
  FollowupStatus,
  FollowupSource,
  FollowupSourceType,
  FollowupClosureReason,
  ContactEventType,
  ContactDeliveryStatus,
  ContactEvent,
  FollowupRuleRow,
  OrganizationFollowupSettings,
} from "./followups";
