...
- **Estado en producción (25-sep)**: mig **270 aplicada** y verificada en la base (RPC `inventory_assign_unlotted`, `inventory_lot_assignments` con RLS, `lot_assignment_id`, CHECKs `asignacion_lote`, guarda del kardex, GRANT a `authenticated`). Mig **271 pendiente**: hasta aplicarla, "Editar producto" avisa que falta la actualización de base de datos; el resto de Almacén funciona.
