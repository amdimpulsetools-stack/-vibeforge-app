...
- [Changelog — Sesión 2026-09-07 → 08 (v0.15.41) — App Review de Meta enviado + idempotencia WhatsApp (mig 249) + dinero en Reportes (migs 250/251) + Almacén Fase A (mig 252) + Agenda "Liberar hueco" (mig 253)](#changelog--sesión-2026-09-07--08-v01541--app-review-de-meta-enviado--idempotencia-whatsapp-mig-249--dinero-en-reportes-migs-250251--almacén-fase-a-mig-252--agenda-liberar-hueco-mig-253)
...
### Tabs Vencimientos y Rentabilidad reales (13-ago, adelantados de F2/F3)
El founder preguntó por qué decían "próximamente" — eran fases de septiembre, pero los datos ya existían desde la mig 209, así que se construyeron en el acto:
...
### Dos fixes de configuración que se ignoraba en silencio (13-ago)
Ambos detectados por el founder preparando la cuenta de la piloto:
- **El modal post-cita copiaba SIEMPRE el mensaje de fábrica**, no el configurado en Settings → WhatsApp → Post-cita. Desde la mig 139 la plantilla vive en la BD por organización (`org_whatsapp_clipboard_templates`) y Settings la guarda ahí, pero `whatsapp-clipboard-modal` seguía leyendo la copia de `localStorage` que ya nadie escribe → caía al texto por defecto. Verificado contra prod: la plantilla personalizada de la piloto estaba guardada correctamente y el modal la ignoraba. Ahora el modal la pide a la BD al abrir (con skeleton y botones bloqueados el instante que tarda, para que nadie copie el texto viejo), la cachea en `localStorage` para que la siguiente cita sea instantánea, y ese caché pasa a ser el fallback offline en vez del texto de fábrica.
- **El correo de prueba fallaba con "Datos inválidos"** sin decir por qué: el cliente mandaba `logo_url`/`clinic_name` como `null` (JSON.stringify no omite null) y el validador solo aceptaba string, `undefined` o `""` — una org sin logo no podía enviar la prueba. Reproducido con el esquema real. Fix en tres capas: el esquema acepta null, la ruta lo normaliza, y el cliente ya no envía claves nulas, valida asunto/cuerpo antes de llamar al API y **muestra el detalle de validación** que el servidor ya devolvía y el toast descartaba.

### Fix del editor de texto enriquecido: el cursor saltaba al final al escribir (13-ago)
Reportado por el founder configurando las plantillas de la piloto; diagnosticado con agente Opus 5. **Causa raíz** (`components/rich-text-editor.tsx`): el editor emitía el HTML **saneado** al padre pero el efecto de sincronización lo comparaba contra el HTML **crudo** de TipTap. El saneo normaliza (`<br>`→`<br />`, `&nbsp;`→carácter, estilos sin espacios), así que la comparación **nunca converge**: cada tecla disparaba `setContent` del documento entero y ProseMirror mandaba la selección al final. Se activaba con el primer espacio (o desde la primera tecla en plantillas con saltos de línea, como el correo de bienvenida de trial). La pestaña "Código" ya tenía un parche local del mismo problema; la "Visual" nunca lo recibió.
- Fix: el efecto ahora pregunta "¿este valor lo emití yo?" (`lastEmittedRef`) en vez de comparar byte a byte, y jamás pisa el DOM con el editor enfocado — un cambio externo real solo ocurre sin foco (al cargar otra plantilla).
- Beneficia también a las **plantillas clínicas** (`clinical-templates-tab`), que usan el mismo componente y sufrían lo mismo desde el montaje.
- De paso: `previewData` y la sustitución de variables de la vista previa memoizadas (corrían en cada tecla aunque la preview estuviera cerrada).
- Verificado que el commit de la vista previa con datos reales (2a46315) **no** causaba ni agravaba el bug: es anterior y vivía por completo en el editor.

### Auditoría matemática del módulo (agente experto Fable 5, 13-ago) + fixes
El founder pidió auditar la matemática y el flujo con un agente experto en gestión de almacenes. Veredicto: camino feliz sólido (CPP verificado a mano, kardex append-only real, CHECKs sin huecos de signo, redondeo inmaterial), pero **el gesto "Deshacer" rompía dos invariantes financieros**. Corregido en el acto:
- **CRÍTICO 1 — Deshacer una entrada contaminaba el CPP para siempre** (10@100 + 10@200 deshecha → CPP quedaba 150 en vez de 100 y toda venta posterior estampaba COGS inflado). Fix: el contra-asiento hereda el `unit_cost` del original y `avgCostByProduct` des-promedia los ajustes valorizados; los contra-asientos viejos sin costo lo heredan del movimiento que reversan.
- **CRÍTICO 2 — Los movimientos deshechos nunca se neteaban en los KPIs** (una venta anulada inflaba "Ventas"/"Ganancia" para siempre; una entrada deshecha, "Compras"). Fix: `reversedPairIds` — todo agregado monetario (Rentabilidad, totales y gráfica de Movimientos) excluye el par original+contra-asiento; el kardex los sigue mostrando porque son historia.
- **MEDIO — Pérdida por merma invisible**: nueva KPI en Rentabilidad (Σ costo congelado de mermas del período) — vencer 5 viales a CPP S/400 ya no desaparece del estado de resultados.
- **MEDIO — Capital valorizado con método mixto**: "Capital en stock", plata dormida y el fallback de Vencimientos ahora valorizan al **CPP** (antes: última compra, hasta +38% sobreestimado mientras el COGS usaba promedio).
- **MEDIO — Tope de 5,000 movimientos**: banner rojo cuando se alcanza (antes truncaba los movimientos más antiguos y el stock mentía en silencio).
- **MENORES**: `saldo_inicial` ya no cuenta como "Compras" del período; el toast de Deshacer calculaba el stock restaurado sobre un closure viejo (decía 12 cuando eran 10) — ahora lo deriva de la lista nueva.
- **Diferidos a F2 (anotados)**: COGS estampado en servidor (RPC/trigger — hoy un snapshot de cliente desactualizado puede congelar un CPP viejo si otra usuaria compró entre medio), tope de asignación automática de lote (saldos de lote negativos ocultos), costo exigible en ajustes positivos grandes, y descontinuar solo con stock 0.
...
## Changelog — Sesión 2026-08-15 (v0.15.32) — Módulo Caja certificado (migs 213-215) + Farmacia POS certificada (migs 216-217) + fixes de aritmética NubeFact

Día de dos módulos nuevos **certificados en producción**. Arrancó con una evaluación de **4 agentes especialistas** sobre Caja + Farmacia + ficha de paciente (sobre código y datos de producción; síntesis en el artifact "Caja y Farmacia") y cerró con Caja (F3) y Farmacia POS (F4) en prod, certificados por el founder. Decisiones de arquitectura que salieron de la evaluación: venta POS con `source='pos'` (la deuda clínica no se contamina), devoluciones como **movimientos de caja** (nunca pagos negativos), vinculación pago→turno **por trigger** (jamás rechaza un cobro) y retiro del motivo "Venta" manual de Almacén al activar el POS.

### Facturación F2 (PR #284) — 2 bugs de aritmética en la emisión NubeFact + validación SUNAT + refactor de proveedor
- **Bug (a) — IGV por unidad×cantidad**: el error de redondeo crecía con la cantidad. Ahora el IGV se calcula **por línea, por diferencia exacta**.
- **Bug (b) — descuento global restado solo del total**: sobre-declaraba IGV. Ahora se **prorratea a las líneas con regla de mayor resto**.
- Validación SUNAT: **boleta ≥ S/700 sin DNI se bloquea ANTES de reservar correlativo**.
- Histórico de comprobantes visible aunque la org esté desconectada del proveedor.
- Refactor: NubeFact confinado a `nubefact-provider.ts` con tablas de traducción — prepara YendaFact/un segundo proveedor tras la interfaz existente.
- **37 tests numéricos** (`npm run test:einvoice`).

### Módulo Caja F3 (migs 213-215 en prod, PR #285) — CERTIFICADO
- Addon `caja` **S/19/mes, incluido desde Centro Médico** (`included_from_plan='professional'`), en beta oculta con grants a las 2 orgs del founder + Patricia (patrón mig 207).
- Turnos con fondo inicial, **arqueo ciego configurable**, movimientos con signo (egreso/sangría/ingreso/devolución), **diferencia solo sobre efectivo** (los medios electrónicos son conciliación informativa), diferencia fuera de tolerancia **exige motivo (CHECK en la base)**, turno cerrado **inmutable** (trigger que bloquea UPDATE/DELETE de sus pagos incluso a service_role), bandeja **"Fuera de turno"** con atribución, y cierre forzado por admin marcado como tal.
- Trigger `caja_stamp_payment`: sella **autor + medio** en cada pago y lo ata al turno abierto — **nunca rechaza un cobro** (sin turno abierto, el pago cae a la bandeja Fuera de turno).
- **CERTIFICADO por el founder con 6 tests en producción**: aritmética, doble apertura, descuadre, inmutabilidad vía SQL admin, fuera de turno.

### Módulo Farmacia POS F4 (migs 216-217 en prod, PR #286) — CERTIFICADO
- Vive **bajo el addon `almacen`** (sin addon nuevo). Venta borrador→confirmada→anulada con **correlativo NV-**, paciente opcional (público general).
- Carrito de 2 columnas con **FEFO** y totales en vivo (**misma aritmética que facturación**).
- RPC `pharmacy_confirm_sale` **transaccional e idempotente**: advisory locks, CPP calculado en servidor, kardex con `sale_line_id UNIQUE` (anti doble descuento de stock), pago `source='pos'` atado a caja por trigger.
- RPC `pharmacy_void_sale`: contra-movimientos + devolución en turno abierto; **sin caja abierta no revierte**.
- Ticket interno **80mm "NOTA DE VENTA"** con leyenda de no-comprobante SUNAT.
- `get_patient_summary` filtra `source='clinical'`: la **deuda clínica no se contamina** con ventas POS — verificado en prod.
- Retiro del motivo "Venta" del modal de Almacén → botón **"Vender → Farmacia"** (se cierra el doble camino).
- **79 aserciones + prueba de concurrencia real**. **CERTIFICADO por el founder**: venta a público general, venta a paciente y anulación completa. **PR #287** (fix del diálogo de cobro que reaparecía) pendiente de merge.

### Decisiones comerciales — la facturación se terceriza por ahora
- Cada clínica abre su **cuenta NubeFact directa** (S/70/mes tarifa pública) y Yenda cobra el **addon de integración**.
- **Reseller NubeFact evaluado** — dictamen: **NO firmar con 3 clínicas**; renegociar con adenda a ~20-30 clínicas. Cotización **CT-65011** guardada.
- **Comparativa de facturadores** entregada: shortlist **APISUNAT / eFact / Bizlinks** + cuestionario de 13 preguntas.
- YendaFact (motor propio) ya está documentado en COMING-UPDATES — sin cambios hoy.

### Datos — reset del almacén de Patricia
- Stock a **cero** (36 productos conservados; movimientos y lotes de prueba eliminados) para la carga de su inventario real.

### Roadmap próximo (F5-F7)
- **F5**: emisión NubeFact desde el POS + comprobante mixto (probar en Vitra). La mig 213 ya dejó `sunat_product_code`/`unit_of_measure`/`igv_affectation`/`is_sellable` en `inventory_products`.
- **F6**: secciones de productos en la ficha de paciente (diseño listo del agente UI): "Productos aplicados" en el tab Clínico, "Productos vendidos" en Finanzas con estado de anulación, **sin séptima pestaña**, fix del overflow de tabs.
- **F7 (higiene)**: bug de Cupos extra que deja el módulo gratis al cancelar, Rentabilidad **neta de IGV** (hoy sobreestima el margen ~18%), normalizar el vocabulario de `payment_method` en patient-drawer y budgets-panel, FK `patient_id`→SET NULL, regenerar `types/database.ts`.
...
PRs de la sesión: #343 (precio en sidebar + mapa del dinero), #344 (mig 250), #345 (mig 251 + Presupuestos oculto a recepción), #346 (desglose colapsable), #347 (desglose flotante + Almacén Fase A, mig 252), #348 (Agenda mig 253 + docs Fase B), #349 (fix vista Día), #350 (bloqueos auditados + Break Time por org, mig 254), #351 (Historia Clínica oculta a recepción), #352 (color por servicio, mig 255).
...
- **Segunda vuelta con la doctora (mig 251)**: la tarjeta "Total facturado" (precio de las citas atendidas, S/ 750) no la leía nadie y generaba confusión frente a "Citas del periodo" (S/ 2 050). Decisión del founder: **"Facturado por citas" pasa a ser lo cobrado en el rango sobre las citas del rango** —incluidos los pagos añadidos a la cita—, y desaparece la tarjeta de precio. Para que la pantalla entera diga lo mismo, `doctors[].collected` (mig 251) alimenta la tarjeta, el gráfico "Facturación por doctor", la tabla de productividad y el export; Σ por doctor == `collected_breakdown.period_appointments`. Se conserva `doctors[].revenue` en el RPC por si un día vuelve la medida de producción. Tarjetas finales: **Facturado por citas · Cobrado total (con desglose) · Pendiente por cobrar**, más Cobros por tratamientos cuando aplica.
- Registro anómalo detectado en la clínica de Patricia: un inicio de FIV cobrado (S/ 1 200) sobre una cita con precio S/ 0. Ese cobro pertenece al módulo Tratamientos, que la clínica empieza a usar el miércoles 9-sep; hasta entonces "Facturado por citas" lo incluye.
- Desglose de "Cobrado total" como **panel flotante** abierto desde un chevron en la esquina de la tarjeta (PR #347): ni abierto ni plegado desnivelaba ya la fila de siete tarjetas.

### Almacén Fase A — precio de venta sin entrada + costo por lote (mig 252)
- **Caso de la clínica**: el laboratorio sube la lista y todavía queda stock. El precio de venta solo se podía cambiar al registrar una entrada, así que había que inventar un lote para subir el precio. Y el modal de lotes mostraba código y vencimiento, pero no a cuánto se compró cada uno (el dato estaba guardado desde la mig 209).
- **Editar precio de venta** desde la tabla (icono de etiqueta, solo owner/admin) → modal con precio actual, precio nuevo, motivo opcional, **margen neto estimado** con el precio nuevo (÷1,18 según afectación − costo promedio, misma fórmula de Rentabilidad) y el **historial de precios** que el trigger de la 209 llevaba escribiendo desde el primer día. Mig 252: RPC `inventory_set_sale_price` que actualiza el producto y estampa el motivo en la fila de historial (la tabla no tiene policy de UPDATE para hacerlo desde el cliente). Sin la mig, el cliente degrada al update directo, sin motivo.
- **Costo por lote** visible en el modal de lotes: "Comprado a S/ X sin IGV · proveedor".
### Agenda: "Liberar hueco" al finalizar antes + hora de fin editable — mig 253
- **Caso de la Dra. Patricia (8-sep)**: consulta de 45 min que termina a los 20. "Finalizar" guardaba la hora real de cierre pero el bloque seguía ocupando sus 45 min en el calendario y en el control de choques: imposible encajar una imprevista o rodar la siguiente. La duración solo se fijaba al crear (mig 221); Reprogramar conserva la duración y Editar no la tocaba.
- **Liberar hueco** (siempre preguntando, decisión del founder): al finalizar antes de la hora de fin, un aviso ofrece "Liberar hueco desde HH:MM"; y mientras quede bloque, la píldora de estado (tarjeta y sidebar) muestra la acción. Acorta `end_time` a "ahora" en el **reloj de la clínica**, redondeado al **siguiente múltiplo de 5 min** (10:22 → 10:25; nunca al intervalo de la agenda), solo el mismo día de la cita y nunca por debajo de inicio + 5. Mismo permiso que Finalizar.
- **Reserva online: no de momento.** Mig 253 añade `appointments.online_busy_until` (hora de fin original al liberar); `/api/book/[slug]` sigue mostrando el bloque ocupado hasta esa hora. La agenda interna y "Horarios disponibles" (copiar para WhatsApp) sí ven el hueco al instante.
...
- **Modalidad por cita: presencial / virtual — mig 256** (PR #353): pedido de la Dra. Patricia para no duplicar servicios ("Primera consulta de fertilidad" presencial y virtual). **Hallazgo**: la app no guardaba si una cita era virtual, lo deducía de que tuviera link de reunión; con un servicio "Ambos" el formulario rellenaba solo el link del doctor y toda cita salía con camarita y con la plantilla de confirmación virtual aunque fuera presencial. Ahora `appointments.modality` (in_person / virtual; NULL = cita antigua, se deduce como antes: decisión del founder, las viejas quedan como están). Fuente única de lectura `lib/appointment-modality.ts` (`resolveAppointmentModality`, `serviceDisplayName`). Al crear/editar una cita de un servicio "Ambos" aparece un **selector inline Presencial / Virtual sin opción marcada** y no deja guardar hasta elegir; el link del doctor solo se rellena si es virtual. Tarjeta (Día, Semana, móvil), sidebar e historial muestran "Servicio · Virtual" con la camarita; la confirmación y los recordatorios eligen la plantilla virtual por modalidad; la reserva online pregunta al paciente cuando el servicio es "Ambos" y el servidor la valida. Para todas las orgs, sin gate.
- **Color por servicio en la agenda — mig 255** (PR #352): pedido de la Dra. Patricia para distinguir sus consultas de los procedimientos. Hasta hoy la tarjeta tomaba todo su color del doctor. Ahora el servicio puede tener un color opcional (misma paleta de 10 que los doctores) desde un desplegable discreto "Color en la agenda" en el formulario del servicio; en la lista de servicios sale un punto con el color. Con color, el **fondo y el texto** de la tarjeta lo usan y el **borde izquierdo conserva el color del doctor** (quién y qué en la misma tarjeta). Sin color = idéntico a hoy. Los tintes de llegó / en consulta / finalizada siguen mandando. Vista Día, Semana escritorio y lista de Semana en móvil. Reserva online, reportes y WhatsApp no cambian. Pendiente si lo piden: atajo "pintar toda la categoría".
- **Recepción ya no ve "Historia Clínica" en el sidebar de la cita** (PR #351): el botón salía a todos los roles y abría la nota en solo-lectura. Misma regla que la pestaña "Clínico" de la ficha del paciente: solo admin o un usuario con ficha de médico. Pendiente de endurecer en BD: la policy de lectura de `clinical_notes` (mig 050) sigue siendo "cualquier miembro de la org".

### Agenda: bloqueos con autor y desbloqueo auditado + Break Time por org — mig 254
- **Pregunta del founder**: ¿solo quien bloqueó puede desbloquear? Auditoría con evidencia de prod: la barrera no era la autoría sino el **rol** (policy de DELETE de la mig 031: solo owner/admin). Doctor y recepción veían el candado, pulsaban, PostgREST devolvía 204 sin error al filtrar RLS, y la app decía "Horario desbloqueado" sin haber hecho nada. `created_by` existía desde la mig 009 pero nunca se rellenaba (0 de 37 bloqueos en prod con autor) y el desbloqueo era un DELETE físico sin rastro.
- **Ahora**: cualquier miembro activo desbloquea. Desbloquear = **marca** (`removed_at`, `removed_by`, `removed_by_name`), nunca borrado; el bloqueo guarda quién lo creó (`created_by` con default `auth.uid()` + `created_by_name`). Bloquear y desbloquear pasan por `/api/scheduler/blocks` (POST) y `/api/scheduler/blocks/[id]` (DELETE) y dejan fila **"Bloqueo de agenda · Crear / Eliminar"** en `clinical_access_log` (tipo nuevo `schedule_block`), visible en Administración → **Registro de auditoría** con el detalle "08/09/2026 · 10:00–12:00 · Motivo · bloqueado por Melissa". El DELETE físico sigue siendo de owner/admin.
- **Agenda**: el slot bloqueado muestra en tooltip y en el menú del candado "Bloqueado por Melissa · 08/09 10:15"; el desbloqueo comprueba que tocó una fila y avisa con la verdad (sin permiso / ya no existe / ya quitado). En el slot no queda nada: al desbloquear vuelve a ser un hueco normal, decisión del founder.
- **Break Time pasa del navegador a la org** (`scheduler_settings.break_time`, jsonb): antes vivía en el localStorage de cada usuario, así que la recepcionista lo veía y la doctora en su celular no, y **el servidor no podía aplicarlo**: "Compartir horarios" y la reserva online ofrecían las franjas del descanso. Ahora lo ve todo el equipo igual; solo owner/admin lo cambian (el resto lo abre en modo lectura); `/api/scheduler/available-slots` y `/api/book/[slug]` lo descartan, y la ocupación del dashboard lo descuenta como la de la agenda. La config guardada en el navegador sirve de placeholder hasta que baja la fila de la org: **hay que guardarlo una vez desde la agenda** para que quede en la clínica.
- **Reserva online**: al confirmar, el servidor vuelve a comprobar bloqueos vigentes y Break Time (antes solo comprobaba citas: un POST manual podía reservar en un bloqueo). "Hoy" y el rango de fechas en el reloj de la clínica (mig 240) en vez de UTC.
- **Compartir horarios**: "hoy" y "ahora" en el reloj de la clínica: después de las 19:00 Lima el servidor (UTC) ya estaba en mañana y dejaba de descartar las horas pasadas de hoy.
- Todos los lectores filtran `removed_at IS NULL` (agenda, sidebar al alargar, reserva online, compartir horarios, ocupación del dashboard); sin la mig aplicada repiten la consulta sin el filtro. Fuera de alcance, anotado: la vista Semana sigue sin botón de desbloqueo.
...
## Changelog — Sesiones 2026-09-09 → 11 (v0.15.42) — Recetas a recepción (migs 257/258) + Presupuestos sin paso intermedio (mig 259) + "Resumen de cobros" imprimible (mig 260)

PRs de la sesión: #353 (modalidad por cita, mig 256), #354 (sufijo "· Virtual" también en tarjetas compactas), #355 (recetas: impresión, firma, Dosis única, catálogos unificados, aviso a recepción; migs 257/258), #356 (Iniciar tratamiento directo desde Pendientes, mig 259), #357 (botones del presupuesto en una fila), #358/#359 (conceptos de pago: alineación + toggle Activo), #360 (fix de la franja gris al final de la página), #361 (análisis y diseño del "Resumen de cobros" + V1 del reporte, mig 260).

### Meta App Review: APROBADO (15-sep)
- Meta aprobó `whatsapp_business_messaging`, `whatsapp_business_management` y `public_profile` con acceso avanzado, el mismo día del envío (15-sep 04:12 PET). Bastaron los dos screencasts (uno por permiso) sin grabar el Embedded Signup, como decía la documentación oficial. El expediente de WhatsApp Cloud API (app publicada, negocio verificado, proveedor de tecnología verificado, `config_id`, flujo PR #327) queda completo.
- Siguiente: conectar el primer número real de una clínica por Embedded Signup y validar el `register` en Coexistence y el `postMessage` del signup, que en el PR #327 quedaron defensivos por no poder probarse sin acceso avanzado. Después, aviso de caducidad del token a 60 días con reconexión en un clic.

- **Post-aprobación, mismo día**: el Embedded Signup seguía en bucle y luego decía "no puede registrar clientes". Diagnóstico con el MCP de Meta (cero llamadas a Graph: nada llegaba a Yenda) y revisión de código con un agente. **Causa**: el registro como *Independent Tech Provider* (Casos de uso → WhatsApp → Convertirte en socio) nunca se había iniciado; es un trámite aparte de la verificación de acceso y del App Review. Completado por el founder ("2 de 2 pasos"): el Builder muestra los tres requisitos en verde y permite lanzar v4.
- Código (PR del 15-sep): `extras.version` v4 por env (`NEXT_PUBLIC_META_ES_VERSION`, default `v4`; antes corría v2, que muere el 15-oct-2026); Coexistence v3/v4 (`FINISH_WHATSAPP_BUSINESS_APP_ONBOARDING` sin `phone_number_id`: se resuelve listando los números de la WABA); CSP de /settings con `staticxx.facebook.com` (sin él el `code` nunca llega al opener) y recarga única si la navegación suave trajo la CSP global; trazas `[wa-es]` en consola. Expediente: `docs/meta-app-review.md` §5b.
### Recetas: impresión legible, firma al pie, "Dosis única" y aviso a recepción — migs 257/258 (PR #355)
- **Pedido de la Dra. Patricia** tras imprimir sus primeras recetas con el motor HTML: la firma quedaba pegada al último medicamento y no al pie; la razón social (`REPROFERTILIDAD E.I.R.L.`) salía debajo del logo y no la quiere en un documento clínico; faltaba la frecuencia "Dosis única"; el pie "Fin de la receta" salía dos veces; y el botón "Crear receta" seguía activo con la receta ya emitida.
- **Membrete**: `buildOrgDocBlock` acepta `doc.hide_org_name`; receta y orden de examen lo activan. Con logo cargado se imprime el logo y el resto del membrete (dirección, teléfono, RUC) sin nombre fiscal; sin logo, sale el nombre comercial de la org para que el papel no quede sin identidad. Presupuestos no cambian.
- **Firma**: bloque de firma anclado al pie de la última página (línea, nombre, CMP y sello). Dosis única: catálogo de frecuencias unificado en `types/clinical-history.ts` (`PRESCRIPTION_FREQUENCIES` + `SINGLE_DOSE_FREQUENCY`) y `lib/prescriptions/format.ts` como único formateador: el compositor rápido de la cita y la Historia Clínica ya no tenían las mismas listas (la divergencia real era en vías: "Subcutánea" vs "Subcutánea (SC)"), ahora es una sola.
- **Atajos de la cita** (`components/clinical/clinical-shortcuts.tsx`): con receta emitida el atajo pasa a "Ver receta" + un "+" para una nueva; el icono "Receta asignada" de la tarjeta se repinta al instante (prop en el comparador del memo + `onSaved` en el sidebar).
- **Aviso a recepción V1** (`docs/spec-receta-a-recepcion.md`): evento `prescription_issued` en el catálogo de notificaciones en vivo (apagado por defecto, lo enciende cada org en Administración) que avisa a recepción "Receta emitida para {paciente}" sin asumir que la paciente compra en la misma clínica (matiz del founder). Mig 257: `prescriptions.medication_catalog_id` (nullable, SET NULL) para no perder el vínculo con el catálogo que el compositor ya calculaba y tiraba; la V1 no lo lee, se guarda desde hoy para que la V2 (puente con Farmacia) arranque con historia.
- **Mig 258 (higiene, hallazgo colateral)**: la policy `prescriptions_update` (mig 053) era org-wide y sin `WITH CHECK`: 6 recepcionistas de prod podían editar una receta firmada por PostgREST. Ahora UPDATE solo owner/admin/doctor, el mismo criterio que el servidor aplica al crear. Verificada en prod tras aplicarla (`prescriptions_update | true`).
- Evaluación legal pendiente de cierre: si la receta impresa exige firma manuscrita en Perú (agente de investigación sin informe final; el PDF deja línea de firma y sello).

### Presupuestos: "Iniciar tratamiento" directo desde Pendientes — mig 259 (PRs #356/#357)
- **Pedido del founder**: "Marcar como aceptado" → tab Aceptados → "Iniciar tratamiento" eran dos clics y un cambio de pestaña para una sola decisión. Desde Pendientes ahora se decide **Iniciar tratamiento** o **Marcar rechazado** y el presupuesto pasa directo a «En curso». Los botones del presupuesto quedaron en **una sola fila** (#357).
- El bloqueo estaba en BD: `treatment_start_from_budget` (mig 245) exigía `accepted`. Mig 259 admite `pending_acceptance` y `accepted` (los 3 presupuestos aceptados sin iniciar de prod siguen pudiendo arrancar) y estampa `accepted_at` si faltaba. Riesgo de orden de deploy explicado y resuelto: la mig se aplicó **antes** del merge (`guarda_relajada | true`); sin ella el botón nuevo devolvía 23514.

### Conceptos de pago y franja gris (PRs #358, #359, #360)
- `/admin/treatment-concepts`: campos y selects en la misma línea; "Activo" pasa a **toggle** en la misma fila (el `<p>` de la clave forzaba una fila nueva de la grilla; el toggle va antes en el DOM).
- **Franja gris al final de cualquier página**: el checkbox oculto del toggle (`sr-only` = `position:absolute`) no tenía ancestro posicionado, escapaba del scroller de `main` y estiraba el documento (alto 1478 vs ventana 862, medido en consola por el founder). Primera hipótesis (unidades `dvh`) descartada con evidencia (`dvh: true`); fix real: `relative` en el `<label>`. Se conservan los fallbacks `dvh → vh` en `@layer base`.

### Caja activada para la clínica de la Dra. Patricia
- Addon `caja` insertado en `organization_addons` por SQL del founder (script preparado aquí, nunca ejecutado desde la sesión contra prod).

### Tratamientos: doctora + asesora ve todos los tratamientos — mig 261
- **Caso real (11-sep, clínica de la Dra. Patricia)**: Verenisse es obstetra (rol doctor, con ficha de médico) y además asesora de fertilidad; es quien recibe la noticia del embarazo y el único tratamiento en curso la tiene asignada como asesora. No lo veía: `treatments_caller_role` (mig 245) etiqueta primero como `doctor`, y un doctor solo ve los tratamientos donde él es la doctora. La combinación doctora + asesora se quedaba con lo más restrictivo. Recepción y la owner sí lo veían.
- **Decisión del founder**: doctora que además es asesora ve e interactúa con TODOS los tratamientos de la clínica (lista, detalle, KPIs, cierre), como una asesora sin ficha. Un doctor a secas sigue viendo solo los suyos. Honorarios visibles para ella (es doctor; el presupuesto que arma ya los imprime).
- Mig 261: helper `treatments_doctor_scoped(p_org_id)` (true solo para doctor sin bandera de asesora) como única condición de alcance; `treatment_close` y `get_treatments_overview` con el cuerpo verbatim de la 245 salvo esa condición. Misma condición en `/api/treatments` (lista) y `/api/treatments/[id]` (detalle). Sin cambio de tablas ni policies. Banco de pruebas desechable 15/15 (doctor solo · doctora+asesora · asesora sin ficha · recepción · cierre ajeno) y rollback verificado.

### Captación: revisión pre-lanzamiento, cohortes por fecha y un número por org — mig 262
- **Contexto (15-sep)**: con el Tech Provider registrado, Captación (migs 206/207) va a recibir tráfico real. Revisión completa con un agente (webhook, capturador, RPC, RLS, pantalla, privacidad). Veredicto: captura sólida (idempotencia por wamid, referral congelado al primer contacto, captura aislada que nunca rompe recordatorios), pero con huecos antes del primer número real.
- **Webhook sin firma**: si faltaba `WHATSAPP_APP_SECRET` solo avisaba y procesaba igual (cualquiera podía insertar leads con service role; ya estaba en la revisión de seguridad del 1-sep). Ahora acepta `WHATSAPP_APP_SECRET` o `META_APP_SECRET` (es el mismo secreto de Meta) y en producción **rechaza con 500** si no hay ninguno. Documentado en `.env.local.example`.
- **Un número activo por org** (mig 262): índice único parcial en `whatsapp_config(phone_number_id) WHERE is_active`; el capturador solo mira configs activas. Verificado en prod antes de la mig: una sola fila con número (DemoClinic), entra limpio.
- **Lo que pidió el founder**: "cuántos de los que escribieron hace tres meses reservaron una cita", filtrable por fechas. El RPC medía una ventana fija (últimos 30 días, citas en los 30 días siguientes). `captacion_summary(p_org_id, p_from, p_to)` trabaja ahora por **cohorte del primer mensaje** en la fecha civil de la org, y cuenta citas, asistencias y cobros hacia adelante **sin tope de días**. Pantalla con presets 30 / 90 / 6 meses / 12 meses / personalizado.
- **Honestidad de los números**: facturado solo con cobros clínicos (antes sumaba farmacia del POS); **leads nuevos vs pacientes ya conocidos** (con Coexistence entran también los chats de pacientes de siempre: lead = sin ficha previa al primer contacto, tolerancia 1 día), y agendaron/asistieron/facturado se cuentan solo sobre leads nuevos; fila "Sin anuncio (orgánico)" para que la tabla sume lo mismo que los KPIs; cruce por teléfono compartido toma el paciente más reciente (antes arbitrario); fuera la alerta roja "N leads sin respuesta" (nada escribía ese estado: siempre era el total). Fechas en la zona de la org, membresía activa exigida en la API. Meta envía titular e ID del anuncio, no el nombre de la campaña (eso exige la Marketing API: pendiente). Banco de pruebas desechable 18/18 + rollback.
- **Nombre propio por anuncio (mig 263)**: Meta solo envía el ID y el titular del anuncio, nunca el nombre de la campaña ("1era consulta video 2"); traerlo exige el permiso `ads_read` (otro App Review) y conectar la cuenta publicitaria de cada clínica. Decisión del founder: la clínica le pone el nombre desde Yenda con un lápiz en la tabla (owner/admin). Tabla `captacion_ad_labels` por org × ad_id con RLS; el RPC devuelve `label` y la pantalla muestra nombre propio, titular e ID. `PUT /api/captacion/ad-labels` (vacío = borrar).
- Pendiente anotado: purga y plazo de retención de los textos de WhatsApp (Ley 29733, va con la política de IA), y confirmar con el primer número real qué entrega Meta por webhook en Coexistence.

### Almacén: archivar o eliminar producto — mig 264
- **Caso real**: "Pergoveris 75 UI" quedó creado por la carga masiva sin un solo movimiento y no había forma de sacarlo de la lista desde la app (se borró por SQL). Y al revés: un producto que ya rotó y se dejó de vender se quedaba en la tabla para siempre. Revisión previa con un agente: la baja lógica ya existía desde la mig 209 (`is_discontinued`, `discontinued_at/by/reason` con CHECK que exige motivo, índice único de nombre por org PARCIAL sobre activos) y la mig 212 bloquea descontinuar con stock ≠ 0; los movimientos son append-only y las FKs de movimientos/lotes/ventas son RESTRICT.
- **Decisión del founder**: un producto CON historial (movimientos, lotes, ventas del POS, comprobantes) NUNCA se borra: se **archiva** y se puede **restaurar**. Un producto SIN nada de eso se **elimina** de verdad, con auditoría. Archivar NO desvincula el catálogo de medicamentos (`medication_catalog.inventory_product_id`): el vínculo es historia y vuelve al restaurar; el chip "Farmacia" del catálogo se oculta mientras el producto esté archivado. No se añaden `is_active` ni `archived_at`: se reutiliza la baja lógica de la 209.
- **Mig 264**: tabla `inventory_product_deletions` (snapshot jsonb del producto + historial de precios + ids del catálogo enlazados; RLS lee owner/admin, escribe solo el RPC). RPC `inventory_archive_or_delete_product(p_product_id, p_reason)` gateado con `is_org_admin` (42501): cuenta movimientos/lotes/ventas/comprobantes; todo en 0 → auditoría + DELETE (`{action:'deleted'}`); si no → archiva con motivo obligatorio (≥ 3 caracteres) → `{action:'archived', movements, lots, sales, invoices}`. Si el trigger 212 bloquea por stock ≠ 0 → `check_violation` legible "Tiene N unidades en stock: registra una salida o un ajuste a 0 antes de archivar". `FOR UPDATE` sobre el producto: cualquier venta que lo referencie espera a que termine la transacción, así el conteo y el DELETE no se cruzan. RPC `inventory_restore_product`: si mientras estaba archivado se creó otro activo con el mismo nombre (el índice es parcial) → `unique_violation` legible "Ya existe otro producto activo con ese nombre; renómbralo antes de restaurar". Harness SQL local 28/28 + rollback.
- **API** `POST /api/almacen/products/[id]/archive` (`{reason?}`) y `/restore`: sesión, membresía activa owner/admin en la org del producto, RPC con el cliente del usuario. 42501 → 403, 23514/23505 → 409 con el mensaje del RPC, P0002 → 404.
- **UI Almacén**: menú "⋯" por fila (escritorio y móvil, solo owner/admin) con "Archivar", que se llama "Eliminar" solo cuando el kardex en memoria permite afirmar que el producto está virgen (0 movimientos, 0 lotes; con el kardex al tope de carga se ofrece "Archivar" y el RPC decide). Modal `archive-modal.tsx` (patrón price-modal): explica "Tiene N movimientos y M lotes: se archivará, no se borrará" o "No tiene historial: se eliminará definitivamente", motivo obligatorio solo al archivar, aviso con stock ≠ 0. Chip **"Archivados (n)"** que lista los archivados (fecha y motivo) con acción **"Restaurar"**. La página carga TODOS los productos: la tabla, Entrada y Salida reciben solo activos; Movimientos, Vencimientos (ya excluía archivados con stock 0) y Rentabilidad reciben todos, así una venta histórica de un producto archivado sigue contando. El POS no cambia (ya excluía archivados). Cadenas nuevas con claves ES/EN en `language-provider` (`almacen.archive.*`).

### Landing: sellos de confianza en el hero, sin demo
- **Pedido del founder (15/16-sep)**: fuera el botón "Agenda una demo de 20 minutos" del hero (sigue en la sección del motor de seguimientos); los dos CTAs ("Empezar mis 14 días gratis" y "Calcula cuánto pierdes hoy") en una sola fila en ≥sm; y bajo "Sin tarjeta. Sin contrato…" un bloque de **credenciales** (`components/landing/trust-credentials.tsx`, agente de UI) con los logos oficiales como SVG inline (la CSP de la landing no admite imágenes de CDN): **App verificada por Google** (G multicolor · "Verificación OAuth aprobada", 28-ago) y **Proveedor de tecnología · WhatsApp Business Platform** (glifo de WhatsApp · "Registro aprobado por Meta", 15-sep). Dos tarjetas con marco y "+" en las esquinas, apiladas en móvil. Cumplen la regla de la landing (solo afirmaciones verificables) y las guías de marca de Google y WhatsApp para indicar integración: sin "oficial", sin "partner", sin el badge Meta Business Partner ni el logo de Meta.

### Presupuestos: "duplicate_budget" crudo al registrar desde el panel
- **Caso real (14-sep, clínica de la Dra. Patricia)**: Karen tiene un FIV pendiente de respuesta y se le quiso registrar una Criopreservación desde el panel de Presupuestos → toast "duplicate_budget". El servidor (`/api/budgets/assign`) no bloquea: devuelve 409 con los activos y pide `acknowledged_existing`. El modal de la ficha del paciente (`AssignBudgetModal`) ya lo maneja con el aviso ámbar y el check "es adicional"; el modal del panel (`BudgetRecordModal`) no lo implementó y mostraba el código crudo.
- Ahora el modal del panel carga los presupuestos activos al elegir la paciente, muestra el mismo bloque (tratamiento · tier · estado · antigüedad) con el check de confirmación, envía `acknowledged_existing` y, si aun así llega un 409 (carrera con otro usuario), lo muestra y vuelve a pedir el check. Los demás errores muestran `message` en vez del código.

### Agenda: Reprogramar con hora libre, misma lógica que al crear la cita
- **Pedido del founder (15-sep)**: el modal Reprogramar del sidebar tenía un desplegable de 15 en 15 min acotado al horario de apertura. No se podía mover una cita de 09:00 a las 07:00 si la agenda abre a las 07:15, y una cita a las 14:20 (caso Vitra) ni siquiera aparecía en la lista: el desplegable pintaba la primera hora. Crear cita, en cambio, siempre aceptó cualquier hora.
- Ahora Reprogramar usa el mismo campo de hora libre en pasos de 5 min y el mismo aviso azul informativo "Fuera del horario de apertura de la agenda… no se ofrece por reserva online", que nunca bloquea. La hora de fin sigue derivada de la duración original (acortar o alargar es tarea de Editar, mig 253). Los choques con consultorio, doctor, bloqueos y Break Time no cambian: ya se evaluaban sobre cualquier hora. Guardas nuevas para hora vacía y fin pasada la medianoche, imposibles antes con el desplegable.

### Reportes › "Resumen de cobros" — reporte personalizado imprimible, mig 260 (PR #361)
- **Pedido de la Dra. Patricia**: una hoja por rango de fechas con lo cobrado en cuatro tablas (Descripción · Cantidad · Precio · Total) —Servicios, Adelantos y pagos a cuenta, Farmacia, Cobros por tratamientos— y un TOTAL FINAL de lo que marque. Cuatro agentes (UI, UX, datos, reportes modernos) analizaron primero contra datos reales de prod (1–10 sep: 72 cobros, S/ 32 892; "Control prenatal" con 3 precios distintos; dos cobros FIV sobre citas a S/ 0). Diseño en `docs/spec-reporte-personalizado.md` + anexos en `docs/reporte-personalizado/`. Aprobado por el founder con las recomendaciones de la sección 9.
- **Regla de dinero respetada (un número, una fórmula)**: el reporte no inventa una definición de "cobrado". RPC `get_custom_report(p_org_id, p_from, p_to, p_sections)` (mig 260) copia **verbatim** las CTEs de `get_reports_overview` (mig 251); cada sección ES una cubeta de `collected_breakdown`: Servicios = `period_appointments`; Adelantos = `other_appointments + other + plans` (los planes van dentro de Adelantos, sub-grupos adelanto de cita futura / pago atrasado / abono directo / plan); Farmacia = `pharmacy`; Tratamientos = `treatment_payments_amount`. Con las cuatro marcadas, TOTAL FINAL == `payments_amount + treatment_payments_amount` del Financiero para el mismo rango, y la pantalla lo verifica en vivo (línea de cuadre verde; ámbar si difiere). Montos brutos con IGV por fecha de cobro; devoluciones de Caja informativas, fuera del total. **Precio**: el real si es único en el grupo, "varios (mín – máx)" si difiere, nunca un promedio. Ventas anuladas de farmacia se muestran como fila aparte (su cobro sigue en `patient_payments`, mig 232; excluirlas queda para V1.1 junto con `get_reports_overview`). Harness SQL 38/38.
- **Solo owner/admin** (gating en el RPC con `get_user_org_role`, 42501 → 403). Pestaña **"Resumen de cobros"** en /reports (segunda posición, hereda el rango del encabezado, sin botón de IA): chips para marcar secciones, tarjetas por sección con "Atendidas/Producción" informativas en Servicios, TOTAL FINAL con "excluye…" cuando hay secciones sin marcar. Secciones no aplicables a la org (módulo apagado y S/ 0) no se pintan: funciona igual en una clínica sin Farmacia ni Tratamientos.
- **Contrato único** `types/custom-report.ts` para RPC, `GET /api/reports/custom` (Zod, "hoy" en la zona horaria de la org, `Cache-Control: private, no-store`) y **PDF con membrete** `GET /api/pdf/custom-report` (mismo motor Handlebars → Chromium de los demás documentos, paginado por sección). `lib/format-pen.ts` (`formatPEN`) como formateador único de soles para lo nuevo.
- Sin la mig 260 aplicada la pestaña muestra el error del servidor ("función no existe") sin tocar el resto de Reportes; no hay orden de deploy crítico, pero la mig va primero para que la doctora no vea el aviso.

### Resumen de cobros: detalle de adelantos y abonos sin cita — mig 265
- **Caso (17-sep)**: en "Adelantos y pagos a cuenta" los abonos sin cita salían en UNA fila fija ("Abono directo (sin cita asociada)") y los cobros de citas de otras fechas por servicio: para cuadrar con la contadora había que abrir ficha por ficha para saber de quién era cada abono y por qué se cobró.
- **Decisión del founder**: lista de **detalle por cobro** (fecha, paciente, monto, medio, motivo; en citas de otras fechas además fecha de la cita y servicio; en planes el título) que sale de la **misma CTE** que ya calcula los totales (`range_payments`), así Σ detalle == `by_bucket` por construcción. Las filas agregadas (`rows`) no cambian. Mig 265: `CREATE OR REPLACE` de `get_custom_report` con la MISMA firma (sin DROP); `range_payments` expone `patient_id/payment_method/payment_date/created_at`; nueva CTE `adv_detail` (`kind` con el mismo criterio que las filas agregadas; `LEFT JOIN patients` → "Paciente sin ficha" si el pago no tiene ficha); `sections.advances.detail` (orden `payment_date, created_at`, LIMIT 300) + `detail_truncated`. Rollback = cuerpo exacto de la 260.
- **Banco de pruebas** `docs/reporte-personalizado/sql/` (espejo regenerado desde la mig, `run.sh` ahora resuelve la raíz del checkout desde su propia ruta): todas las aserciones previas + Σ `detail[kind='direct']` == `by_bucket.other`, Σ `detail` == `advances.total` y comprobaciones de campos por `kind`.
- **Pantalla** (`reports/custom-report.tsx`): subfilas desplegables bajo cada sub-grupo con "Ver N cobros / Ocultar"; **Abono directo abierto por defecto**, Cita futura / Cita anterior / Anticipo a plan plegados; aviso cuando la lista viene recortada a 300. i18n ES/EN (`reports.custom_detail_*`).
- **PDF**: subtabla SOLO bajo "Abono directo" (fecha dd/mm · paciente · motivo recortado a 60 caracteres, "—" si vacío · medio · monto), 8.4 pt atenuada con sangría y `break-inside: avoid`; tope 300 filas + "y N más" calculado contra la cantidad de la fila agregada (exacto aunque el RPC recorte). **La impresión queda auditada** con `logClinicalAccess` (`resource_type 'other'`, `action 'print'`, metadata con rango, secciones y nº de cobros del detalle): nombre + motivo es dato personal (Ley 29733), como la receta.
- **Cobro en la ficha del paciente**: "Motivo / referencia" pasa a ser **obligatorio (mín. 3 caracteres) SOLO sin cita asociada** ("-- Ninguna --"), con el mensaje "Indica el motivo del abono (mín. 3 caracteres)" y botón deshabilitado; con cita sigue opcional. Solo validación de UI (no hay ruta API para ese insert; sin CHECK en BD: el histórico tiene NULL).

---

## Changelog — Sesiones 2026-09-18 → 22 (v0.15.43) — Landing paralela /2 + permiso de almacén por miembro (migs 266/267) + auditoría de Sentry

PRs de la sesión: #370 (landing paralela `/2`, mergeada el 18-sep), #371 (Almacén: quién puede crear productos, migs 266/267).

### Landing paralela `/2` — PR #370
- **Pedido del founder (18-sep)**: iterar la estructura de la landing sin mover la URL original hasta que esté lista. Se crea `/2` como variante paralela: **misma información, marca y precios** que la home, otra organización de secciones (referencia externa aportada por el founder, reconstruida desde su CSS/JS porque el `index.html` no venía).
- **Secciones** (`components/landing-v2/`): header con anclas a la propia página · hero con **mockup de la app** (pestañas Agenda / Pacientes / Caja y dos notas flotantes: asistente IA y cita recuperada) · franja de esenciales con las cuatro credenciales reales · **motor de seguimientos como pieza central** (Detecta / Contacta / Mide con demo que cambia al elegir cada paso, navegable por teclado) · roles del equipo con el asistente IA pegado · impacto con calculadora · prueba "Hecho en Perú" · precios con toggle de cadencia · FAQ con `<details>` nativos · cierre y footer.
- **Una sola fuente para el dinero**: los tres planes salen a `components/landing/pricing-plans.ts` y la fórmula de recuperación a `components/landing/roi-model.ts`; **la home ahora los importa** (estaban duplicados en `pricing.tsx`, `revenue-impact.tsx` y `pain-quiz.tsx`). Sin cambio de comportamiento: mismos precios, mismo modelo 20 %→12 % + 4 %.
- **Aislamiento**: `/2` registrada en `publicPaths` del middleware, `robots: { index: false }` en la página, excluida de `app/sitemap.ts` y añadida al `disallow` de `app/robots.ts` — no compite con la home mientras se compara. Analítica: los mismos tres eventos (`cta_trial_click`, `cta_demo_click`, `perfil_select`) con `variante: "2"` para separar embudos.
- **Reglas respetadas** (brief + auditoría 21-ago): H1 fijo, sin botón de demo en el hero (la demo vive en el motor de seguimientos), "te avisa para contactarla en un clic" y nunca "la contacta automáticamente", sellos con la redacción exacta aprobada (Google / Meta), el ancla de precio solo en Precios, narrativa de recuperación en femenino, sin guiones largos como coma.
- Verificación: `tsc` y `next build` en verde (`/2` estática, 20.9 kB vs 35.1 kB de la home); capturas con Playwright a 390/820/1024/1440 px sin desbordamiento horizontal, un solo `h1`.
- **Hallazgo preexistente (no de este PR)**: 11 de los 12 `.woff2` de `app/fonts/` son páginas HTML "Error 404" guardadas con extensión de fuente desde el merge del PR #319. El build imprime "Unknown font format" por cada uno y **toda la app cae a la fuente del sistema**, no solo `/2`. Pendiente de reemplazar por los WOFF2 reales de Plus Jakarta Sans, Outfit y JetBrains Mono.

### Almacén: quién puede crear productos — migs 266/267 (PR #371)
- **Caso (21-sep, clínica de la Dra. Patricia)**: Verenisse (obstetra, rol `doctor`) no veía el botón "Producto". Causa doble: la UI gateaba con `isOrgAdmin` y la policy de escritura de `inventory_products` (mig 209) también era owner/admin. Entradas y salidas sí podía registrarlas (esas policies admiten a cualquier miembro). Las obstetras son quienes rellenan y actualizan el almacén.
- **Primer intento (mig 266)**: abrir la escritura al rol `doctor`. Helper `is_org_inventory_editor`, policies INSERT/UPDATE para editores y DELETE solo admin, RPC `inventory_set_sale_price` con el nuevo gate.
- **Corrección del founder, mismo día**: la regla no es por rol sino **por miembro aprobado**. Mig 267: columna `organization_members.can_manage_inventory` (default `false`); el helper pasa a "owner/admin activo, **o** miembro activo con el permiso", así que las policies y el RPC de la 266 no se tocan. **Solo el owner concede o quita el permiso**: se amplía el trigger anti-escalada de la mig 235 (misma técnica con la que reserva el rol owner), de modo que un admin no puede concedérselo ni concederlo a otros aunque la policy de UPDATE de `organization_members` sea `is_org_admin`. El permiso sirve igual para una obstetra con rol doctor que para una recepcionista que maneje stock, sin cambiarle el rol.
- **API**: `PATCH /api/members/[id]` acepta `can_manage_inventory` (403 si el llamante no es owner; el trigger lo vuelve a comprobar en BD). `GET /api/members` lo devuelve.
- **Pantalla**: Admin → Equipo muestra un interruptor "Puede gestionar el almacén" por miembro, **visible solo para el owner** y solo en miembros que no son owner/admin (esos lo tienen siempre); optimista, con reversión y aviso si la API lo rechaza. `OrganizationProvider` expone `canManageInventory` (espejo del helper de BD) y Almacén lo usa para "Producto" y "Editar precio de venta". Archivar, eliminar y restaurar siguen con `isOrgAdmin`.
- Ambas migraciones aplicadas en producción y verificadas en `pg_policies`; Verenisse quedó con el permiso activo y el resto de miembros en `false`.
- **Colisión de numeración resuelta**: el número 265 se usó dos veces en paralelo — `265_get_custom_report_detail` (Resumen de cobros, mergeada a main el 17-sep) y la de almacén, creada desde un checkout que todavía no la tenía. Las de almacén se renumeraron a **266/267** (archivos, rollbacks y todas las referencias en código). Nota operativa: en producción quedaron registradas con las etiquetas viejas (`265_inventory_doctor_writes`, `266_inventory_member_permission`) porque se aplicaron antes del renombre; la `version` real es el timestamp, así que **no hay que re-aplicar nada**.

### Sentry: auditoría del estado real y reparto con Vercel (22-sep)
- **Pregunta del founder**: Vercel ofrece su integración de Sentry — ¿configurar por ahí o por código? Auditoría del estado actual antes de decidir.
- **El SDK ya está integrado** desde antes: `@sentry/nextjs ^10.43.0`, `withSentryConfig` en `next.config.ts`, los tres `sentry.{client,server,edge}.config.ts` e `instrumentation.ts` con `onRequestError`. Todo está **gateado por variable de entorno**: sin `SENTRY_DSN` (server/edge) ni `NEXT_PUBLIC_SENTRY_DSN` (cliente) el SDK no inicializa. Hoy **Sentry no recibe ni un evento**: ambas están vacías.
- **Bloqueante para el cliente — la CSP**: `connect-src` (`lib/supabase/middleware.ts`) permite `'self'`, Supabase, Anthropic y Mercado Pago; el ingest de Sentry (`https://oXXXX.ingest.*.sentry.io`) no está. Poner hoy `NEXT_PUBLIC_SENTRY_DSN` haría que **el navegador bloquee cada evento en silencio**. Dos salidas: añadir el dominio de ingest a `connect-src`, o activar `tunnelRoute` en `withSentryConfig` (los eventos salen por nuestro propio origen, cubierto por `'self'`, y de paso esquiva ad-blockers; exige añadir esa ruta a `publicPaths` para que también reporten landing y login). **Decisión pendiente del founder.**
- **Source maps**: `sourcemaps.disable: !process.env.SENTRY_DSN` — poner el DSN **enciende la subida**, que necesita `SENTRY_AUTH_TOKEN` + `SENTRY_ORG` + `SENTRY_PROJECT`. Ninguna de las tres existe hoy en el repo ni en `.env.local.example`. Sin ellas el build no falla, pero los stack traces llegarían minificados e inservibles.
- **Reparto acordado**: la **integración de Vercel se usa solo para credenciales** (crea/enlaza el proyecto de Sentry e inyecta el auth token y org/project en las variables del proyecto; el DSN se verifica después y se añade a mano si no lo inyectó, con los nombres exactos que lee el código). **La configuración de código se queda en el repo**: `sentry.client.config.ts` está afinado a mano (sin tracing de navegador, ~30 kB gz menos; Replay cargado en `requestIdleCallback` para no romper la CSP; `bundleSizeOptimizations`). **Nunca correr `npx @sentry/wizard`**: sobrescribe esos archivos y revierte el ajuste de bundle de la v0.15.27.
- **Deprecación pendiente**: el build avisa que `sentry.client.config.ts` debe migrar a `instrumentation-client.ts` (requisito de Turbopack).

### Tratamientos: monto acordado editable + limpieza de un duplicado en prod (22-sep)
- **Caso real (clínica de la Dra. Patricia)**: una IIU de Rosaly M. quedó registrada **dos veces** el 21-sep, con 12 minutos de diferencia: la owner asignó presupuesto e inició tratamiento a las 18:31, y la asesora repitió la operación completa a las 18:43 (presupuesto nuevo incluido). Los dos cobros (S/300 + S/2,450 = **S/2,750**) cayeron en el segundo. Además ambos figuraban con **S/3,750 acordado** cuando lo pactado eran S/2,750.
- **Causa del monto — no era un bug**: el precio viaja por copias, no por enlaces vivos. `service_budget_tiers.amount` (tier A) → `budget_records.amount` al asignar → `treatments.expected_total` al iniciar (mig 242: "snapshot de budget.amount"). Cambiar el precio hoy NO reescribe hacia atrás, y eso es correcto: un presupuesto ya impreso y entregado es un compromiso, no un valor vivo.
- **Causa de que "no se actualizara" al cambiar el precio — sí era un problema de UX**: el founder corrigió el precio en `/admin/services`, pero editó **`services.base_price`** (el precio de la CITA). Los presupuestos leen `service_budget_tiers` (`app/api/budgets/assign/route.ts`). Con `pricing_mode='single'` la pantalla muestra los dos precios y nada decía cuál manda: el tier A siguió en 3,750 (intacto desde el 7-jul) y **el siguiente presupuesto habría vuelto a salir mal**. El founder lo corrigió en el campo correcto tras el análisis (tier A = 2,750, verificado en prod).
- **Corrección de datos en prod** (respaldo completo de las filas guardado antes de tocar nada): monto acordado y presupuesto del tratamiento superviviente a 2,750; Verenisse asignada como asesora (la traía el duplicado, no él); duplicado y su presupuesto **borrados**. Se descartó etiquetarlo: los únicos estados posibles son `rejected` y `expired`, ninguno describe "creado por error" y `rejected` habría falseado a la baja la conversión de la asesora en los reportes. El PDF del presupuesto borrado sigue en Storage.
- **Monto acordado editable** (`app/(dashboard)/tratamientos/[id]/page.tsx`): lápiz junto a "Acordado" que abre un campo inline con guardar/cancelar, Enter y Escape. Solo owner/admin, el mismo gate que ya tenía `PATCH /api/treatments/[id]` desde la mig 245 — **la API lo soportaba desde siempre y la UI nunca lo llamaba**, así que cualquier corrección exigía SQL a mano. Mensaje de error de la API tal cual ("Solo dirección puede cambiar el monto acordado").
- **Desambiguación en `/admin/services`**: bajo el precio base, cuando el servicio es cotizable, aviso de que ese precio cobra las citas y que presupuestos y tratamientos usan el de más abajo; y el copy de "Precio del tratamiento" ahora dice que ese monto se copia al presupuesto y al tratamiento y que **cambiarlo no afecta a los ya emitidos**.
- Pendiente evaluado y NO hecho: evitar el duplicado de raíz (dos personas iniciando el mismo tratamiento con minutos de diferencia). Un índice único por paciente+servicio+estado abierto rompería los casos legítimos de repetir un ciclo; la vía sensata es un aviso al iniciar cuando ya existe un tratamiento abierto igual para esa paciente.

### Sentry cableado de verdad: CSP, `instrumentation-client.ts` y source maps (22-sep)
- Cuenta creada por el founder en sentry.io (org `yenda-s5`, proyecto `javascript-nextjs`, Next.js autodetectado desde el repo de GitHub). **El asistente `npx @sentry/wizard` NO se ejecutó, a propósito**: sobrescribe los tres configs afinados a mano y revertiría el recorte de bundle de la v0.15.27 (sin tracing de navegador, Replay perezoso, `bundleSizeOptimizations`).
- **CSP — el bloqueante real** (`lib/supabase/middleware.ts`): `connect-src` no incluía el ingest de Sentry, así que con el DSN puesto el navegador habría descartado **cada evento en silencio**. El origen se deriva del propio `NEXT_PUBLIC_SENTRY_DSN` (`new URL(dsn).hostname`, en try/catch: un DSN mal formado no puede reventar la CSP entera) y se añade a las **tres** políticas mediante un helper `connectSrc(...)`, que además deduplica la base común que estaba escrita tres veces. Sin DSN no se añade nada. Se descartó `tunnelRoute`: abre un POST público que proxea a Sentry y suma una invocación de función por evento; el allowlist explícito es el mismo patrón ya usado para Culqi y Facebook.
- **Deprecación resuelta**: `sentry.client.config.ts` → **`instrumentation-client.ts`** (requisito de Turbopack; el build avisaba en cada compilación).
- **Privacidad explícita, no heredada del default**: `sendDefaultPii: false` en cliente, servidor y edge, y `replayIntegration({ maskAllText, maskAllInputs, blockAllMedia })` en vez de las opciones por defecto. Son los valores por defecto del SDK, pero la política de privacidad **ya publicada** promete "Configurado con scrubbing de datos sensibles" y la pantalla que graba el Replay es la historia clínica de una paciente: una promesa legal no puede depender de un default que el SDK cambie en una versión mayor.
- **Source maps**: `withSentryConfig` no declaraba `org` ni `project`, así que con el DSN puesto la subida se habría saltado con un warning y los stack traces llegarían minificados. Ahora van con default (`yenda-s5` / `javascript-nextjs`, no son secretos) y `SENTRY_ORG`/`SENTRY_PROJECT` los sobrescriben. El único secreto sigue siendo `SENTRY_AUTH_TOKEN`, que inyecta la integración de Vercel.
- `.env.local.example` documenta las cinco variables y para qué sirve cada una.
- **Una sola DSN**: al instalar la integración Sentry↔Vercel se vio que crea `NEXT_PUBLIC_SENTRY_DSN`, `SENTRY_AUTH_TOKEN`, `SENTRY_ORG` y `SENTRY_PROJECT`, pero **no** `SENTRY_DSN`. Server, edge y el gate de source maps (`sourcemaps.disable`) leían solo `SENTRY_DSN`, así que con la integración puesta el navegador habría reportado y el servidor no, y los mapas no habrían subido nunca. Ahora los tres hacen `SENTRY_DSN ?? NEXT_PUBLIC_SENTRY_DSN`: la `NEXT_PUBLIC_` basta, la otra queda como override. El DSN es público por diseño (viaja en el HTML), leerlo en servidor no expone nada.
- **Pendiente del founder**: instalar la integración Sentry↔Vercel (crea las variables y el token de source maps) y redesplegar. Después, confirmar la llegada del primer evento y decidir si el Session Replay se queda encendido (graba la pantalla, enmascarada, en una app clínica).

---

## Changelog — Sesión 2026-09-22 (v0.15.44) — Tratamientos: insumos de la propia farmacia + cierre "Completado" + fuentes repuestas (mig 268)

PRs de la sesión: #373 (Sentry cableado, mergeado), #374 (fuentes `.woff2` repuestas), y el PR de esta entrada (mig 268, tratamientos).

### Sentry: primer evento real en producción — PR #373
- El commit del cableado se empujó **después** de fusionar el #372, así que main tenía el monto acordado editable pero no la CSP con el ingest ni `instrumentation-client.ts`. Se rebasó sobre main y salió como PR aparte (#373). Al instalar la integración Sentry↔Vercel se vio que crea `NEXT_PUBLIC_SENTRY_DSN`, `SENTRY_AUTH_TOKEN`, `SENTRY_ORG` y `SENTRY_PROJECT` pero **no** `SENTRY_DSN`: servidor, edge y el gate de source maps leían solo esa. Ahora los tres hacen `SENTRY_DSN ?? NEXT_PUBLIC_SENTRY_DSN` (el DSN es público por diseño, leerlo en servidor no expone nada). `.env.local.example`, CHANGELOG y PRD reflejan la variable única.
- **Verificado en producción** con el founder: `window.__SENTRY__` presente, DSN en el bundle (`main-app-*.js`), `fetch` manual al ingest → HTTP 200 (la CSP nueva deja pasar), y `throw new Error("Prueba de Sentry desde Yenda")` desde la consola → issue `JAVASCRIPT-NEXTJS-2`, *Unhandled*, con usuario. La primera prueba no llegó porque se lanzó mientras Vercel aún construía el deploy del #373.
- Añadido `export const onRouterTransitionStart = Sentry.captureRouterTransitionStart` en `instrumentation-client.ts` (el SDK lo pedía con `ACTION REQUIRED` en cada build; sin él los errores al navegar entre páginas del App Router se quedaban sin la ruta destino).
- Notificaciones: no hay nada que instalar en el código; email por defecto (regla "new issue" del proyecto), app móvil de Sentry para push, y se recomendó una regla de "más de 10 usuarios afectados en 1 h" + *spike protection* (plan gratuito: 5 000 eventos/mes).

### Fuentes: 11 de 12 `.woff2` eran páginas 404 en HTML — PR #374
- Confirmado con el build local y con la consola de producción (`OTS parsing error: invalid sfntVersion: 1008813135` = los bytes `<!DO`): desde el PR #319, `app/fonts/` tenía 11 páginas de error de Google guardadas con extensión `.woff2` (1.6 kB cada una); solo `Outfit-Regular.woff2` era real. `next/font/local` registraba el error y seguía, emitiendo igual el `@font-face`; el navegador descargaba HTML creyendo que era una fuente. **La app llevaba semanas renderizándose con la fuente del sistema** en vez de Plus Jakarta Sans, Outfit y JetBrains Mono (cambian métricas, saltos de línea y alturas de fila).
- Repuestas las doce desde Google Fonts (subconjunto `latin`, OFL), verificado glifo a glifo que traen `ñ Ñ á é í ó ú ü Ü ¿ ¡ °`. Build: 0 `Failed to load font file` (antes 11).

### Tratamientos — donantes, insumos de la propia farmacia y cierre "Completado" — mig 268
- **Caso real (Dra. Patricia)**: a una **donante de óvulos** (Shannon N.) se le inicia una criopreservación que no paga (S/ 0) y durante la estimulación se le aplican medicamentos del propio almacén sin cobrárselos; ese dinero se recupera cuando viene la **receptora** de ovodonación (su presupuesto ya incluye la parte de la donante; así están armados los totales desde la mig 180). Análisis previo del código, hallazgos:
  - Almacén ya tenía **"Aplicación"** (`reason_code='uso_en_cita'`: salida con COGS congelado y sin precio, con `patient_id`) pero **no sabía de qué tratamiento** era.
  - La pestaña **Rentabilidad** contaba esas aplicaciones **solo como unidades**: su costo no se restaba de nada (`profit-tab.tsx`). El dinero desaparecía de todas las vistas: bajaba el capital en anaquel, la ganancia de farmacia no se movía y el tratamiento no se enteraba.
  - Una "venta a S/ 0" por el POS era la herramienta equivocada (exige método de pago, crea un `patient_payments` de cero, cuenta como venta con 100 % de descuento). `treatment_external_payments` tampoco (es plata que la paciente pagó a un tercero). El concepto "Medicación" (`third_party`) es para medicación comprada afuera.
  - No existía ningún vínculo donante ↔ receptora en el modelo (queda para la fase 2).
- **Decisiones del founder**: servicio aparte "Criopreservación de óvulos — donante" con precio único S/ 0 (lo crea en `/admin/services`; el presupuesto y el tratamiento nacen en 0, sin editar a mano); cierre **"Completado" sin desenlace clínico**; el vínculo kardex → tratamiento es una columna nullable (no se puede activar por org, pesa cero para quien no la usa; lo que se condiciona es la función: solo dentro de la ficha del Pack Fertilidad y con el módulo Almacén activo); **Melissa (recepción) también aplica** → cualquier miembro activo de la org, misma regla que la policy de INSERT del kardex y que cobrar en el POS.
- **Mig 268**:
  - `treatments.outcome` admite **`completed`** (CHECK recreado con nombre `treatments_outcome_chk`; el de la 242 era inline y se localiza por definición).
  - `inventory_movements.treatment_id` (nullable, FK **RESTRICT** + índice parcial). RESTRICT y no SET NULL: el kardex es append-only por trigger (mig 209) y un SET NULL en cascada lo dispararía; además un tratamiento con insumos no debe poder borrarse. Misma técnica que `sale_line_id` (mig 213).
  - RPC **`treatment_apply_product(p_treatment_id, p_product_id, p_quantity, p_lot_id?, p_movement_date?, p_notes?)`** → `{movement_id, unit_cost, cost_total, warnings[]}`. Mismo movimiento que la Aplicación de Almacén (salida, `uso_en_cita`, `patient_id` de la paciente, **COGS = CPP vigente calculado en el servidor con `pharmacy_avg_cost`**, la función que ya usa el POS: una sola fórmula, no una copia en el cliente; `unit_sale_price` NULL) y además `treatment_id`. Gates: miembro activo, addon fertilidad, módulo Almacén, tratamiento en curso, producto de la org y no archivado, lote del producto. Stock insuficiente **no bloquea** (criterio del POS, mig 217): se aplica y queda el aviso. Lock por producto. **NUNCA toca `patient_payments`, Caja ni deuda**: no es cobro, ni pago, ni pago a tercero.
  - RPC **`treatment_undo_supply(p_movement_id)`**: contra-asiento (ajuste opuesto, `reverses_movement_id`, hereda el costo, sin `patient_id` ni `treatment_id`, patrón `undoMovement` de Almacén), jamás DELETE. Quien puede: editor de almacén (mig 267) o quien registró la aplicación. Rechaza deshacer dos veces.
- **Las matemáticas, regla por regla**: kardex `SUM(quantity)` como siempre · dinero del tratamiento (`treatmentMoney`) **no se mueve** · costo de insumos = `treatmentSuppliesCost()` en `lib/treatments/money.ts` = Σ `cost_total` de las salidas con `treatment_id` **sin los pares deshechos** (neto: el costo de compra viaja sin IGV; `estimated` si alguna fila no tiene costo estampado) · **Rentabilidad de Almacén** no lo resta de la ganancia (la farmacia no perdió esa plata: la clínica la invirtió en un tratamiento que se recupera por otro lado) pero lo muestra en un KPI nuevo **"Aplicado a tratamientos"** para que el capital que salió del anaquel no se esfume · margen neto del tratamiento (`acordado ÷ 1.18 según afectación − terceros − insumos`) queda para la Entrega 2 ya prevista en `money.ts`; para la donante será negativo a propósito. Ejemplo: donante 0/0/0 + insumos 2 550 → margen −2 550; receptora 25 850 (neto 21 906,78) − terceros 6 000 − insumos propios 1 200 − insumos de su donante 2 550 = 12 156,78; farmacia: ventas sin cambio, "Aplicado a tratamientos" 3 750, capital en anaquel −3 750. Nada se cuenta dos veces y nada se pierde.
- **API**: `GET /api/treatments/[id]` devuelve `supplies[]` (kardex + producto + lote embebidos; los contra-asientos se buscan aparte por `reverses_movement_id` porque no llevan `treatment_id`, mismo hallazgo que la ficha del paciente F6), `supplies_cost`, `supplies_estimated`, `can_apply_supplies` (en curso + Almacén activo) y `can_undo_supplies` (owner/admin o `can_manage_inventory`; el autor de cada fila también, lo resuelve el RPC). Nueva `POST/DELETE /api/treatments/[id]/supplies` → RPCs con el cliente del usuario. `POST …/close` acepta `outcome: "completed"`.
- **UI**: bloque **"Insumos de farmacia"** en la ficha (lista con producto, cantidad, lote, fecha, costo unitario y total; botón **Aplicar producto**; deshacer con confirmación) · diálogo `treatment-supply-dialog.tsx` (buscador por nombre/SKU de productos activos, lote opcional si el producto los lleva, cantidad con atajos 1-4, fecha civil de la org, nota) · insignia **"Sin cobro (donante u otro acuerdo)"** y sin barra de avance cuando el acordado es 0 (antes parecía abandonado) · opción **"Completado"** en el diálogo de cierre · KPI "Aplicado a tratamientos" en Rentabilidad (`treatment_id` añadido a `MOVEMENT_COLUMNS` e `InventoryMovement`, opcional para no romper el optimistic update de Almacén).
...
### Almacén: unidades "sin lote", lote por defecto correcto y editar producto (migs 270-271)
- **Caso (25-sep, Dra. Patricia)**: Adaptessens mostraba STOCK 18 pero la ventana de lotes "17 und en 1 lote": la unidad 18 entró el 22-sep con el Lote vacío. En esa org: 7 productos con 77 und sin lote y 13 con lotes que suman MÁS que el stock (121 und), porque las ventas del POS y los insumos de productos sin control de lotes bajan el stock pero no el lote. Además no había forma de corregir el nombre de un producto.
- **Lote por defecto (FEFO real)**: `nearestLotByProduct(lots, stockByLot)` ignora lotes sin saldo y desempata por recepción y código. Antes la columna LOTE/VENCE mostraba el lote agotado 25-006 (empate de vencimiento + orden físico de la tabla) y las salidas/mermas y el POS podían descontar de un lote en 0 (Gonapeptyl quedó en −2 con otro lote de 21). Insumos de tratamientos: el lote viene propuesto (el que vence primero con saldo), ya no "Sin lote".
- **Ventana de lotes**: fila **"Sin lote asignado"** con cantidad, fecha y hora del ingreso, autor y costo; botón **Asignar a lote** (existente o nuevo con código y vencimiento). Caso inverso: **"Salieron sin lote"** con **Descontar de un lote** (tras el conteo físico). El RPC `inventory_assign_unlotted` (mig 270) asienta un par de ajustes `asignacion_lote` que netea a cero: no cambia stock, CPP ni rentabilidad; auditoría en `inventory_lot_assignments`; una fila del par no se puede "deshacer" sola. Lotes en negativo se muestran como "Descuadre −N"; el costo del lote se toma del kardex (el lote 25-075 decía S/ 101.60 "sin IGV", que era el precio con IGV; el real es S/ 86.10). La tabla marca "N sin lote" / "lotes +N" en los productos que manejan lotes.
- **Editar producto** (menú ⋯, editor de almacén): nombre, categoría, presentación, stock mínimo y **control de lotes**, con motivo opcional e historial (`inventory_product_changes`, lo llena un trigger). RPC `inventory_update_product` (mig 271): nombre sin espacios dobles y único entre activos; archivados solo owner/admin (botón lápiz junto a Restaurar: resuelve el "renómbralo antes de restaurar"). Actualiza carritos en borrador y la fila importada del catálogo de recetas; nunca ventas confirmadas, comprobantes ni recetas. Fuera a propósito: afectación IGV (reescribiría márgenes pasados) y unidad base.
- **Entrada**: vencimiento sin lote ya no se descarta en silencio (pide el código); un producto con control de lotes o con lotes previos exige el código.
- Pruebas: `supabase/tests/pharmacy/30_inventory_lots_and_edit_test.sql` (42 casos, Postgres desechable, `run.sh`) — stock y CPP intactos tras asignar/descontar/lote nuevo, guardas del kardex, permisos (recepción sin permiso, doctor con permiso, archivados), renombrar sin duplicar, catálogo. `tsc` y `next build` en verde. Rollbacks en `migrations/rollbacks/270_*` y `271_*`.
- **Estado en producción (25-sep)**: mig **270 aplicada** y verificada en la base (RPC `inventory_assign_unlotted`, `inventory_lot_assignments` con RLS, `lot_assignment_id`, CHECKs `asignacion_lote`, guarda del kardex, GRANT a `authenticated`). Mig **271 aplicada** el mismo día y verificada (RPC `inventory_update_product`, `inventory_product_changes` con RLS, trigger `trg_inventory_products_audit`, GRANT solo a `authenticated`).
